...
Archive overlay должен быть **repository-root shaped**: корень zip-архива считается корнем репозитория, а все пути внутри архива должны быть путями относительно корня репозитория. Нельзя заворачивать результат в дополнительную папку с именем задачи, overlay, даты, snapshot или repo.
...
```text
conceptual_translation_glossary_overlay_git12/
  START.md
  AGENTS.md
  work/
  protocols/
...
Если меняется только текущая задача, в корне архива может лежать только нужный repository-relative subtree:

```text
work/
```

Если меняются постоянные проектные файлы, top-level entries всё равно должны быть repository-relative paths, а не папка-обёртка:
...
- если snapshot отсутствует, а файл слишком длинный — дать append patch или запросить snapshot.
...
## Финальное закрытие задачи перед ответом

Перед финальным ответом ChatGPT должен выполнить лёгкую проверку закрытия задачи:

1. Проверить, менялась ли рабочая позиция проекта: принятый процесс, текущий набор рабочих документов, статус главы/пакета, baseline, правила применения overlay или другое состояние, от которого зависит следующая сессия.
2. Если рабочая позиция изменилась, обновить state-файлы:
   - `work/discourse.md` — когда изменился ход работы, принято решение, появился новый риск, human gate или новая рабочая рамка;
   - `work/theory-writing/WORKING_DOCUMENTS_MAP.md` — когда изменилась карта theory-writing документов, их статус, роль или маршрутизация;
   - `work/APPLY_NOTES.md` — когда создаётся archive overlay/delta, меняется baseline, порядок применения, deletion list или состав результата.
3. Собрать overlay/delta, если задача создала или изменила файлы репозитория.
4. В финальном ответе явно перечислить:
   - какие state-файлы обновлены;
   - какие содержательные файлы изменены;
   - если ожидаемый state-файл не обновлён, почему он не был затронут.

Это не требует отдельного `STATE_CLOSEOUT.md`. Если пользователь позже попросит более жёсткий gate, его можно добавить отдельным протокольным решением.

## Commit anchors

Каждый существенный commit или suggested commit message должен иметь anchor:
...
Если ни запись, ни архив не выполнялись, финальный ответ должен явно сказать, что изменений в репозитории нет и архив не собирался.
