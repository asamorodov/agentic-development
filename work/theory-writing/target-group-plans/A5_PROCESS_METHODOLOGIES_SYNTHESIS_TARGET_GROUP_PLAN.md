# Target group plan: A5 — process methodologies synthesis

Статус: рабочий план целевой группы. По этому документу можно собрать исполнительный пакет для несущего фрагмента A5. План не является опубликованным текстом теории.

## 1. Обрабатываемые файлы

```yaml
- path: work/theory-writing/fragments/A5_process_methodologies_synthesis.md
  status: future
  role: primary
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Основной фрагмент: when process becomes an artifact, from plan/spec to GSD/BMAD/PWG/Gas Town."

- path: work/theory-writing/fragments/A5_source_usage.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Источники, фактически использованные for process-methodology ladder and process heaviness risks."

- path: work/theory-writing/fragments/A5_story_anchor_map.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Short story anchors for research-plan, app review, harness, cleanup and architecture/context discipline."

- path: work/theory-writing/fragments/A5_figure_candidates.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Кандидаты на иллюстрации for process ladder and thresholds где extra structure helps or hurts."

- path: work/theory-writing/fragments/A5_open_questions.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Unresolved source gaps, especially around exact GSD/BMAD/Spec Kit external details and non-deep-anchor limits."

- path: work/theory-writing/fragments/A5_degradation_and_duplication_audit.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Audit that A5 is not a catalogue of methods and does not dissolve GSD/BMAD into PWG or Gas Town."
```

Целевая группа не является ``linked-target-edit``: основной текст один; служебные outputs фиксируют выбранные источники, story anchors, risks and source gaps.
...
Story-dossiers are optional and should only support process-as-verifiable-migration or workflow contrast. They must not turn A5 into a new story chapter.
...
Явно известные источники: Spec Kit workflow docs, SPDD/OpenSPDD only as lower-boundary background если needed, Open GSD / GSD docs and repositories, `gsd-core`, `gsd-pi`, GSD Auto Mode, BMAD Method docs, BMAD workflow map, BMAD agents, BMAD implementation readiness/checklist materials, Reversa, OpenSpec, AgentSPEX, PWG/Beads/Gas Town sources for the upper boundary.

Выводимые источники: inline-ссылки, source registers, source notes, external-source tables, method names, tool names, repository links and figure candidates from `SPEC_KIT_METHOD_DOSSIER.md`, `GSD_METHOD_DOSSIER.md`, `BMAD_METHOD_DOSSIER.md`, `PERSISTENT_WORK_GRAPH_MECHANISM_DOSSIER.md`, `GAS_TOWN_METHOD_DOSSIER.md`, `PROCESS_METHODS_COMPARATIVE_SYNTHESIS.md` and `SOURCE_TO_PLAN_MAPPING_AFTER_FULL_REREAD.md`.
...
Создай первый черновик `work/theory-writing/fragments/A5_process_methodologies_synthesis.md`. Композиционная функция фрагмента: сравнительный узел о том, когда процесс сам становится артефактом. Он должен построить лестницу от разговора и research/plan к GSD, BMAD, PWG and Gas Town, не превращая её в обзор методологий. Главная напряжённость фрагмента: process может быть реальным носителем состояния и handoff, а может быть только театром ролей и документов; нужно показать, где проходит эта граница.
...
GSD/Open GSD, BMAD, Reversa/OpenSpec/AgentSPEX, PWG and Gas Town sources открывать только для конкретных distinctions.
...
Перечитай `work/theory-writing/fragments/A5_process_methodologies_synthesis.md` и усили его фактурой по направлению: усилить GSD/Open GSD and BMAD as lightweight/role-based process state, with human gates and recovery.

Открывай внешние источники только под конкретные утверждения текста. Если добавляешь новый внешний источник, занеси его в `work/theory-writing/fragments/A5_source_usage.md`: источник, почему добавлен, какое утверждение поддерживает, где использован. Ссылки на внутренние досье, планы и отчёты не являются валидными ссылками в основном тексте.
...
Снова перечитай `work/theory-writing/fragments/A5_process_methodologies_synthesis.md` и добавь недостающую фактуру по направлению: добавить Reversa/OpenSpec/AgentSPEX only where they clarify process-as-artifact boundary.
...
### P04 — граница process / PWG / Gas Town
...
Сделай проход по границам process / PWG / Gas Town. Проверь, не смешал ли текст методологию процесса, долговечный граф работы и организационную среду. GSD и BMAD не должны стать PWG; PWG не должен стать названием методологии; Gas Town не должен превратиться в “более тяжёлый BMAD”.

Исправь только те места, где границы действительно размыты. Сохрани аргумент и ссылки. Если нужно добавить источник под точное различение, открой первоисточник и занеси его в `work/theory-writing/fragments/A5_source_usage.md`.
...
Обязательно открой все новые внешние источники, которые находишь в этом проходе, до того как использовать их. Каждый новый источник занеси в `work/theory-writing/fragments/A5_source_usage.md`. Не добавляй источники для широты и не превращай текст в каталог фактов.
...
Выравняй `work/theory-writing/fragments/A5_process_methodologies_synthesis.md` как несущий фрагмент общей теории. Проверь, ведёт ли он один продолжающийся смысл, не спорит ли со скелетоном, не пересказывает ли истории и не стал ли он оглавлением будущих глав вместо самостоятельного аргумента. Отдельно проверь, не дублирует ли фрагмент A4 про PWG или B3 про Gas Town: здесь нужно объяснить process-as-artifact, а не заново писать соседние узлы.

Сохрани фактуру и ссылки. Убери повторы, слишком локальные рамки и места, где текст выглядит как список тем. Если часть материала лучше вынести в технический атлас, отметь это в `work/theory-writing/fragments/A5_open_questions.md` или `work/theory-writing/fragments/A5_degradation_and_duplication_audit.md`, но не удаляй важную мысль без замены.
...
Приведи `work/theory-writing/fragments/A5_process_methodologies_synthesis.md` к русскому техническому языку по языковому протоколу. Не делай стилевую полировку и не меняй аргумент без необходимости.
...
Повторно проверь русский текст `work/theory-writing/fragments/A5_process_methodologies_synthesis.md`. Ищи гибриды, остаточные английские объяснения, плохие кальки, ошибочно переведённые технические термины и места, где русификация стерла различие между близкими понятиями.
...
Сделай первый стилевой проход по `work/theory-writing/fragments/A5_process_methodologies_synthesis.md`. Текст должен звучать как человеческий технический аргумент: без summary-интонации, без бюрократических списков, без декоративного пафоса и без размазывания сильных различений.
...
Сделай второй стилевой проход. Проверь, читается ли `work/theory-writing/fragments/A5_process_methodologies_synthesis.md` как самостоятельный узел теории, а не как досье, оглавление или пересказ материалов. Усиль переходы, если они нужны, но не добавляй новые факты без источников.

Если история используется слишком широко, оставь её как якорь и перенеси ограничение в `work/theory-writing/fragments/A5_story_anchor_map.md`. Если figure-кандидат не поддержан текстом, уточни его в `work/theory-writing/fragments/A5_figure_candidates.md`.
