# Target group plan: C3 — PWG → evidence

Статус: рабочий план целевой группы. По этому документу можно собрать исполнительный пакет для переходного моста C3. План не является опубликованным текстом теории.

## 1. Обрабатываемые файлы

```yaml
- path: work/theory-writing/fragments/C3_pwg_to_evidence.md
  status: future
  role: primary
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Основной мост: why PWG status must terminate in an evidence package, not in a naked completed flag."

- path: work/theory-writing/fragments/C3_source_usage.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Источники, фактически использованные for status/evidence/oracle/review/replay claims."

- path: work/theory-writing/fragments/C3_story_anchor_map.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Короткие якоря for research trace, acceptance anxiety, enterprise validation and executable workflow evidence."

- path: work/theory-writing/fragments/C3_figure_candidates.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Кандидаты на иллюстрации for work status -> evidence package -> human acceptance."

- path: work/theory-writing/fragments/C3_open_questions.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Пробелы, если A7 или B2 отсутствуют, либо если источникам по test/oracle/replay нужен отдельный проход по первоисточникам."

- path: work/theory-writing/fragments/C3_degradation_and_duplication_audit.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Audit that C3 distinguishes trace, log, test, review comment and evidence package."
```

Целевая группа не является ``linked-target-edit``: мост один. It must not use work status as proof; it must explain why `completed` without evidence package is false completion.
...
Если these fragments are absent, read the corresponding dossiers and write the gap to `C3_open_questions.md`.
...
### Дополнительные story-dossiers and reports
...
Явно известные источники: PWG evidence package materials named in the PWG dossier, TDAD papers and examples, ADR status/decision record sources, contract tests, architecture fitness functions, test-data/oracle sources, Product Migration business-rule oracle and CI loop, Stripe Minions validation loop/devbox materials, Shopify Roast replay/observability/workflow materials.

Выводимые источники: inline-ссылки, source registers, source notes, tool/article/repository names, figure candidates and acceptance/evidence vocabulary from `PERSISTENT_WORK_GRAPH_MECHANISM_DOSSIER.md`, `TDAD_COMPARATIVE_DOSSIER.md`, `ADR_METHOD_DOSSIER.md`, `PRODUCT_MIGRATION_CLAUDE_CODE_STORY_DOSSIER.md`, `PRODUCT_MIGRATION_CLAUDE_CODE_ANTI_DEGRADATION_REPORT.md`, `STRIPE_MINIONS_STORY_DOSSIER.md`, `SHOPIFY_ROAST_STORY_DOSSIER.md` and upstream `A7_source_usage.md` / `B2_source_usage.md` если they exist.
...
Создай первый черновик `work/theory-writing/fragments/C3_pwg_to_evidence.md`. Композиционная функция фрагмента: мост от PWG к evidence. Покажи, что статус работы не должен двигаться сам по себе: `completed` имеет смысл только если рядом есть evidence package, понятный oracle и владелец принятия. Иначе граф работы становится машиной ложного завершения.
...
Источники по PWG, TDAD, ADR confirmation, Product Migration, Roast и Stripe открывай только под конкретные утверждения моста.
...
Перечитай `work/theory-writing/fragments/C3_pwg_to_evidence.md` и усили его фактурой по направлению: добрать examples where observation, tests, review and rollout become evidence attached to work state.

Открывай внешние источники только под конкретные утверждения текста. Если добавляешь новый внешний источник, занеси его в `work/theory-writing/fragments/C3_source_usage.md`: источник, почему добавлен, какое утверждение поддерживает, где использован. Ссылки на внутренние досье, планы и отчёты не являются валидными ссылками в основном тексте.
...
Снова перечитай `work/theory-writing/fragments/C3_pwg_to_evidence.md` и добавь недостающую фактуру по направлению: показать gate/evidence relation without repeating A7.
...
### P04 — проверка переходов состояния
...
Проверь, показано ли в тексте, как evidence управляет переходами состояния работы: ready → in progress → waiting for evidence → review → accepted / rejected / blocked. `completed` допустим только после evidence и authority. Не повторяй A7: здесь нужно не различие observation/evidence само по себе, а прикрепление evidence к work graph.

Открывай внешние источники только под конкретные утверждения. Новые источники заноси в `C3_source_usage.md`; ссылки на внутренние досье не ставь в основной текст.
...
Обязательно открой все новые внешние источники, которые находишь в этом проходе, до того как использовать их. Каждый новый источник занеси в `work/theory-writing/fragments/C3_source_usage.md`. Не добавляй источники для широты и не превращай текст в каталог фактов.
...
Выравняй `work/theory-writing/fragments/C3_pwg_to_evidence.md` как несущий фрагмент общей теории. Проверь, ведёт ли он один продолжающийся смысл, не спорит ли со скелетоном, не пересказывает ли истории и не стал ли он оглавлением будущих глав вместо самостоятельного аргумента.

Сохрани фактуру и ссылки. Убери повторы, слишком локальные рамки и места, где текст выглядит как список тем. Если часть материала лучше оформить как самостоятельный материал или раздел концептуально-технического атласа, отметь это в `work/theory-writing/fragments/C3_open_questions.md` или `work/theory-writing/fragments/C3_degradation_and_duplication_audit.md`, но не удаляй важную мысль без замены и не своди материал атласа к несвязному техническому приложению.
...
### P07 — адресное усиление: наблюдение становится свидетельством
...
Проведи адресный проход по `work/theory-writing/fragments/C3_pwg_to_evidence.md`: усили механику превращения наблюдения в свидетельство. A7 уже разводит наблюдение и свидетельство; C3 должен показать, что PWG даёт место, куда свидетельство прикрепляется: к claim, gate, task, state transition или completion decision. Если свидетельства описаны отдельно от состояния работы, привяжи их к обещаниям изменения и решениям о переходе состояния.
...
### P08 — адресное усиление: завершение через evidence-state
...
Проведи адресный проход по `work/theory-writing/fragments/C3_pwg_to_evidence.md`: проверь завершение через evidence-state. Главная опасность C3 — снова принять зелёный сигнал за завершение. Работа закрывается не потому, что что-то наблюдалось, а потому что нужные свидетельства привязаны к правильным обещаниям изменения и к правильному решению о состоянии. Если текст оставляет эту границу слабой, исправь её.
...
Оцени текущий текст `work/theory-writing/fragments/C3_pwg_to_evidence.md` как готовящийся фрагмент теории: насколько хорошо он выполняет задачу, заданную этим target-group plan, скелетоном и картой документов? Сначала сформулируй проблемы, затем исправь их.
...
Заново оцени уже исправленный `work/theory-writing/fragments/C3_pwg_to_evidence.md`. Не считай первый общий редакторский проход достаточным и не повторяй его механически. Прочитай фрагмент как свежий редактор: насколько он теперь выполняет свою задачу в общей теории? Сначала сформулируй проблемы, затем исправь их.
...
Проведи третий общий редакторский проход по `work/theory-writing/fragments/C3_pwg_to_evidence.md`. Это не языковая и не стилевая полировка. Задача та же: оценить, насколько фрагмент выполняет поставленную задачу, сначала сформулировать оставшиеся проблемы, затем исправить их.
...
Приведи `work/theory-writing/fragments/C3_pwg_to_evidence.md` к русскому техническому языку по языковому протоколу. Не делай стилевую полировку и не меняй аргумент без необходимости.
...
Повторно проверь русский текст `work/theory-writing/fragments/C3_pwg_to_evidence.md`. Ищи гибриды, остаточные английские объяснения, плохие кальки, ошибочно переведённые технические термины и места, где русификация стерла различие между близкими понятиями.
...
Сделай первый стилевой проход по `work/theory-writing/fragments/C3_pwg_to_evidence.md`. Текст должен звучать как человеческий технический аргумент: без summary-интонации, без бюрократических списков, без декоративного пафоса и без размазывания сильных различений.
...
Сделай второй стилевой проход. Проверь, читается ли `work/theory-writing/fragments/C3_pwg_to_evidence.md` как самостоятельный узел теории, а не как досье, оглавление или пересказ материалов. Усиль переходы, если они нужны, но не добавляй новые факты без источников.

Если история используется слишком широко, оставь её как якорь и перенеси ограничение в `work/theory-writing/fragments/C3_story_anchor_map.md`. Для визуального материала не используй правило «кандидат поддерживает аргумент — значит вставить inline» без классификации. Сначала присвой кандидату статус из asset policy: `synthetic_figure`, `local_image_asset`, `external_real_image_candidate` или `editorial_visual_idea`. Inline-вставка допустима только для уже оформленной синтетической схемы, которая прошла usefulness/nontriviality gate, или для разрешённого локального image asset через `<img src="...">`; внешний screenshot/source diagram без asset-pass остаётся в `work/theory-writing/fragments/C3_figure_candidates.md` с причиной и следующим действием. Если кандидат не поддержан текстом, уточни его статус, перенеси в отложенные/отклонённые или сними в `work/theory-writing/fragments/C3_figure_candidates.md`.
