# Target group plan: 00 — spine-map и терминологический контракт

Статус: рабочий план целевой группы. По этому документу можно собрать исполнительный пакет для короткого рабочего фрагмента `00_spine_map.md`. План не является опубликованным текстом теории.

## 1. Обрабатываемые файлы

```yaml
- path: work/theory-writing/fragments/00_spine_map.md
  status: future
  role: primary
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Короткая карта сквозных понятий и терминологический контракт для последующих несущих фрагментов."

- path: work/theory-writing/fragments/00_spine_map_source_usage.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Какие внутренние и внешние источники реально использованы для терминов и рамочных различений."

- path: work/theory-writing/fragments/00_spine_map_story_anchor_map.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Какие истории использованы как проверочные якоря и где запрещён пересказ историй."

- path: work/theory-writing/fragments/00_spine_map_figure_candidates.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Кандидаты на схемы сквозного lifecycle, artifact graph and theory/atlas boundary; ассеты не скачиваются."

- path: work/theory-writing/fragments/00_spine_map_open_questions.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Открытые вопросы, терминологические риски и места, где нужен human gate."

- path: work/theory-writing/fragments/00_spine_map_degradation_and_duplication_audit.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Аудит разрастания spine-map в главу, повторов с A1–A10 и смешения поперечного синтеза теории / концептуально-технического атласа."
```

Целевая группа не является ``linked-target-edit``: основной рабочий текст один. Служебные файлы создаются как следы источников, story anchors, figure candidates, открытых вопросов и деградационного аудита.
...
### Главные материалы для spine-map
...
### Методологические досье для проверки терминов
...
Эти досье не надо читать как материал для подробного пересказа. Они нужны, чтобы spine-map не вводил термины, которые затем противоречат защищённым профилям.
...
### Story-dossiers для точечной проверки
...
Явно известные источники: DORA 2025, Bain `From Pilots to Payoff`, A-SDLC survey, SLR по agentic AI across SDLC, DORA / DevEx / SPACE, Martin Fowler / Xu Hao SPDD, OpenSPDD, Nygard ADR, MADR, Martin Fowler ADR, GitHub Spec Kit, Kiro Specs, Constitutional SDD, оба TDAD, GSD / OpenGSD, BMAD, Beads / Gas Town, Codex / Copilot / Claude Code / Jules docs как platform examples.

Выводимые источники: inline-ссылки, source registers, названия авторов, инструментов, статей, репозиториев, методологий и figure-кандидатов из `work/theory-source-map-ai-driven-sdlc.md`, активных досье and comparative reports. Открывать внешний источник только если spine-map опирается на него в конкретном терминологическом утверждении.
...
Создай первый черновик `work/theory-writing/fragments/00_spine_map.md`. Композиционная функция фрагмента: рабочий терминологический контракт, а не вступление и не краткий пересказ будущей теории. Он должен зафиксировать сквозные понятия, связи и границы theory / stories / atlas так, чтобы остальные узлы не переопределяли их заново. Пиши компактно: не делай оглавление будущих частей, а закрепи язык, которым они будут пользоваться.
...
внешние источники открывать только для рамочных утверждений о AI-driven SDLC / SDLC lifecycle, если такие утверждения входят в текст.
...
Перечитай `work/theory-writing/fragments/00_spine_map.md` и усили его фактурой по направлению: проверить по скелетону и CORE_NODES_WRITING_PLAN, какие понятия должны пройти через все фрагменты: lifecycle, specification, decision memory, PWG, execution, evidence, authority, repair.

Открывай внешние источники только под конкретные утверждения текста. Если добавляешь новый внешний источник, занеси его в `work/theory-writing/fragments/00_spine_map_source_usage.md`: источник, почему добавлен, какое утверждение поддерживает, где использован. Ссылки на внутренние досье, планы и отчёты не являются валидными ссылками в основном тексте.
...
Снова перечитай `work/theory-writing/fragments/00_spine_map.md` и добавь недостающую фактуру по направлению: добавить связи с A1, SPDD, PWG, Gas Town и финальной mode-selection картой; не превращать spine map в оглавление.
...
### P04 — проверка согласованности со следующими узлами
...
Проверь `00_spine_map.md` как контракт для следующих узлов. Он не должен конфликтовать с будущими A/B/C-фрагментами и не должен брать на себя их работу. Особенно проверь термины: specification, decision memory, Persistent Work Graph, execution environment, workflow engine, evidence, authority, lifecycle repair, концептуально-технический атлас.

Правь только места, где карта терминов расползается, становится вступлением, превращается в оглавление или создаёт будущий конфликт. Если для сильного утверждения нужен внешний источник, открой его и поставь ссылку; внутренние досье и планы не являются публичными ссылками.
...
Обязательно открой все новые внешние источники, которые находишь в этом проходе, до того как использовать их. Каждый новый источник занеси в `work/theory-writing/fragments/00_spine_map_source_usage.md`. Не добавляй источники для широты и не превращай текст в каталог фактов.
...
Выравняй `work/theory-writing/fragments/00_spine_map.md` как несущий фрагмент общей теории. Проверь, ведёт ли он один продолжающийся смысл, не спорит ли со скелетоном, не пересказывает ли истории и не стал ли он оглавлением будущих глав вместо самостоятельного аргумента.

Сохрани фактуру и ссылки. Убери повторы, слишком локальные рамки и места, где текст выглядит как список тем. Если часть материала лучше оформить как самостоятельный материал или раздел концептуально-технического атласа, отметь это в `work/theory-writing/fragments/00_spine_map_open_questions.md` или `work/theory-writing/fragments/00_spine_map_degradation_and_duplication_audit.md`, но не удаляй важную мысль без замены и не своди материал атласа к несвязному техническому приложению.
...
### P07 — адресное усиление: терминологический контракт под нагрузкой
...
Проведи адресный проход по `work/theory-writing/fragments/00_spine_map.md`: проверь терминологический контракт под нагрузкой. Не просто убедись, что ключевые термины упомянуты; примени каждый важный термин к SPDD, PWG, Gas Town, свидетельствам, праву действовать/признавать изменение принятым и последующей поддержке артефактов после merge. Если термин работает только как красивое слово, сузь его, замени или перенеси в open questions. Этот проход специальный: он усиливает главную функцию 00 как рабочего контракта языка для всей теории, но не заменяет последующие общие редакторские проходы.
...
### P08 — адресное усиление: spine-map не должен стать оглавлением
...
Проведи адресный проход по `work/theory-writing/fragments/00_spine_map.md`: проверь, что spine-map не стал вступлением, планом книги или оглавлением будущих глав. Текст должен быть рабочим контрактом языка и сквозных различений, которым последующие фрагменты реально пользуются. Убери или перепиши места, где он звучит как “дальше будет A, B, C”, если за этим не стоит самостоятельное понятийное различение. Если материал важен, но принадлежит будущей главе или concept article атласа, зафиксируй это в companion-файлах, не превращая 00 в обзор всего проекта.
...
Оцени текущий текст `work/theory-writing/fragments/00_spine_map.md` как готовящийся фрагмент теории: насколько хорошо он выполняет задачу, заданную этим target-group plan, скелетоном и картой документов? Сначала сформулируй проблемы, затем исправь их.
...
Заново оцени уже исправленный `work/theory-writing/fragments/00_spine_map.md`. Не считай первый общий редакторский проход достаточным и не повторяй его механически. Прочитай фрагмент как свежий редактор: насколько он теперь выполняет свою задачу в общей теории? Сначала сформулируй проблемы, затем исправь их.
...
Проведи третий общий редакторский проход по `work/theory-writing/fragments/00_spine_map.md`. Это не языковая и не стилевая полировка. Задача та же: оценить, насколько фрагмент выполняет поставленную задачу, сначала сформулировать оставшиеся проблемы, затем исправить их.
...
Приведи `work/theory-writing/fragments/00_spine_map.md` к русскому техническому языку по языковому протоколу. Не делай стилевую полировку и не меняй аргумент без необходимости.
...
Повторно проверь русский текст `work/theory-writing/fragments/00_spine_map.md`. Ищи гибриды, остаточные английские объяснения, плохие кальки, ошибочно переведённые технические термины и места, где русификация стерла различие между близкими понятиями.
...
Сделай первый стилевой проход по `work/theory-writing/fragments/00_spine_map.md`. Текст должен звучать как человеческий технический аргумент: без summary-интонации, без бюрократических списков, без декоративного пафоса и без размазывания сильных различений.
...
Сделай второй стилевой проход. Проверь, читается ли `work/theory-writing/fragments/00_spine_map.md` как самостоятельный узел теории, а не как досье, оглавление или пересказ материалов. Усиль переходы, если они нужны, но не добавляй новые факты без источников.

Если история используется слишком широко, оставь её как якорь и перенеси ограничение в `work/theory-writing/fragments/00_spine_map_story_anchor_map.md`. Для визуального материала не используй правило «кандидат поддерживает аргумент — значит вставить inline» без классификации. Сначала присвой кандидату статус из asset policy: `synthetic_figure`, `local_image_asset`, `external_real_image_candidate` или `editorial_visual_idea`. Inline-вставка допустима только для уже оформленной синтетической схемы, которая прошла usefulness/nontriviality gate, или для разрешённого локального image asset через `<img src="...">`; внешний screenshot/source diagram без asset-pass остаётся в `work/theory-writing/fragments/00_spine_map_figure_candidates.md` с причиной и следующим действием. Если кандидат не поддержан текстом, уточни его статус, перенеси в отложенные/отклонённые или сними в `work/theory-writing/fragments/00_spine_map_figure_candidates.md`.
