# Target group plan: A6 — execution environment distinctions

Статус: рабочий план целевой группы. По этому документу можно собрать исполнительный пакет для несущего фрагмента A6. План не является опубликованным текстом теории.

## 1. Обрабатываемые файлы

```yaml
- path: work/theory-writing/fragments/A6_execution_environment_distinctions.md
  status: future
  role: primary
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Основной фрагмент: distinguish execution envelope, tool/sensor surface, workflow engine and platform agent."

- path: work/theory-writing/fragments/A6_source_usage.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Источники, фактически использованные for envelope, tool surface, workflow engine, platform agent and PWG boundary claims."

- path: work/theory-writing/fragments/A6_story_anchor_map.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Короткие якоря for HumanLayer, Sandvault/Homebrew, Pi, Stripe Minions, Shopify Roast and Quix/Klaus Kode."

- path: work/theory-writing/fragments/A6_figure_candidates.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Кандидаты на иллюстрации for four-layer execution environment map and non-equivalence with PWG."

- path: work/theory-writing/fragments/A6_open_questions.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Пробелы в источниках and uncertain boundaries among envelope, tool/sensor surface, workflow engine and platform agent."

- path: work/theory-writing/fragments/A6_degradation_and_duplication_audit.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Audit that A6 does not collapse Roast, Stripe Minions, Quix, Armin and HumanLayer into one agentic environment."
```

Целевая группа не является ``linked-target-edit``: основной текст один. A6 должен различить слои среды исполнения, а не подменить смысловую координацию упоминанием worktree или workflow engine.
...
### Дополнительные источники и соседние материалы
...
Явно известные источники: HumanLayer / 12 Factor Agents, BAML, MCP sources, Claude Code docs, Codex docs, GitHub Copilot cloud agent docs, Jules docs, Sandvault / Mike McQuaid materials, Jesse Vincent Superpowers / hooks / skills, Shopify Roast docs and repository, Stripe Minions sources, Quix / Klaus Kode sources, Armin Ronacher Pi / agentic coding materials, SWE-agent and harness-engineering sources from the source map.

Выводимые источники: inline-ссылки, source registers, source notes, tool names, repository names, figure candidates and screenshot candidates from the five main stories, four story-dossiers, `SOURCE_TO_PLAN_MAPPING_AFTER_FULL_REREAD.md`, `theory-source-map-ai-driven-sdlc.md`, `COMPARATIVE_DEEP_CASES_AUDIT.md` and `STORIES_13_15_LANGUAGE_EDITORIAL_PASS_REPORT.md`.
...
Создай первый черновик `work/theory-writing/fragments/A6_execution_environment_distinctions.md`. Композиционная функция фрагмента: узел различения между средой исполнения, surface инструментов, workflow engine и platform agent. Покажи, что “среда агента” — не один слой: worktree может изолировать действие, но не задаёт workflow; workflow engine может воспроизводить процесс, но не даёт authority; platform agent может встроиться в компанию, но не равен локальному harness; tool surface помогает видеть систему, но не является свидетельством завершения. Именно поэтому Roast, Stripe, Sandvault, HumanLayer and Armin нельзя складывать в один bucket.
...
Claude/Codex worktrees, Sandvault, HumanLayer, Shopify Roast, Stripe Minions, Quix/Klaus Kode and Armin sources открывать только под claims.
...
Перечитай `work/theory-writing/fragments/A6_execution_environment_distinctions.md` и усили его фактурой по направлению: добрать фактуру по worktrees, sandbox, permissions, devbox, logs, browser, MCP and local scripts.

Открывай внешние источники только под конкретные утверждения текста. Если добавляешь новый внешний источник, занеси его в `work/theory-writing/fragments/A6_source_usage.md`: источник, почему добавлен, какое утверждение поддерживает, где использован. Ссылки на внутренние досье, планы и отчёты не являются валидными ссылками в основном тексте.
...
Снова перечитай `work/theory-writing/fragments/A6_execution_environment_distinctions.md` и добавь недостающую фактуру по направлению: добрать фактуру по Roast/Quix/Stripe as workflow/platform layers, без пересказа историй.
...
### P04 — same-story / different-layer pass
...
Сделай same-story / different-layer pass. Возьми 2–4 истории-якоря и проверь, раскладывает ли текст один и тот же case по разным слоям, а не использует историю как цельный пересказ. Например: Stripe Minions как platform agent плюс devbox/validation/review; Shopify Roast как workflow engine and replay; Armin как минимальный harness, logs and local scripts; Sandvault or HumanLayer as execution envelope / harness boundary.

Если это различение не видно, усили текст. Не добавляй новую story-сцену ради иллюстрации; используй только короткие якоря. Все новые внешние источники открывай перед использованием и заноси в `work/theory-writing/fragments/A6_source_usage.md`.
...
Обязательно открой все новые внешние источники, которые находишь в этом проходе, до того как использовать их. Каждый новый источник занеси в `work/theory-writing/fragments/A6_source_usage.md`. Не добавляй источники для широты и не превращай текст в каталог фактов.
...
Выравняй `work/theory-writing/fragments/A6_execution_environment_distinctions.md` как несущий фрагмент общей теории. Проверь, ведёт ли он один продолжающийся смысл, не спорит ли со скелетоном, не пересказывает ли истории и не стал ли он оглавлением будущих глав вместо самостоятельного аргумента.

Сохрани фактуру и ссылки. Убери повторы, слишком локальные рамки и места, где текст выглядит как список тем. Отдельно проверь, не превратился ли фрагмент в каталог сред, CLI and tools, и не смешал ли он execution envelope с PWG, evidence или authority. Если часть материала лучше вынести в технический атлас, отметь это в `work/theory-writing/fragments/A6_open_questions.md` или `work/theory-writing/fragments/A6_degradation_and_duplication_audit.md`, но не удаляй важную мысль без замены.
...
Приведи `work/theory-writing/fragments/A6_execution_environment_distinctions.md` к русскому техническому языку по языковому протоколу. Не делай стилевую полировку и не меняй аргумент без необходимости.
...
Повторно проверь русский текст `work/theory-writing/fragments/A6_execution_environment_distinctions.md`. Ищи гибриды, остаточные английские объяснения, плохие кальки, ошибочно переведённые технические термины и места, где русификация стерла различие между близкими понятиями.
...
Сделай первый стилевой проход по `work/theory-writing/fragments/A6_execution_environment_distinctions.md`. Текст должен звучать как человеческий технический аргумент: без summary-интонации, без бюрократических списков, без декоративного пафоса и без размазывания сильных различений.
...
Сделай второй стилевой проход. Проверь, читается ли `work/theory-writing/fragments/A6_execution_environment_distinctions.md` как самостоятельный узел теории, а не как досье, оглавление или пересказ материалов. Усиль переходы, если они нужны, но не добавляй новые факты без источников.

Если история используется слишком широко, оставь её как якорь и перенеси ограничение в `work/theory-writing/fragments/A6_story_anchor_map.md`. Если figure-кандидат не поддержан текстом, уточни его в `work/theory-writing/fragments/A6_figure_candidates.md`.
