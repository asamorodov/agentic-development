# Target group plan: A4 — Persistent Work Graph boundary

Статус: рабочий план целевой группы. По этому документу можно собрать исполнительный пакет для несущего фрагмента A4. План не является опубликованным текстом теории.

## 1. Обрабатываемые файлы

```yaml
- path: work/theory-writing/fragments/A4_persistent_work_graph_boundary.md
  status: future
  role: primary
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Основной фрагмент: границы Persistent Work Graph как продолжимого состояния работы."

- path: work/theory-writing/fragments/A4_source_usage.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Какие внутренние и внешние источники реально использованы для PWG and adjacent-layer contrasts."

- path: work/theory-writing/fragments/A4_story_anchor_map.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Истории-якоря для work state, handoff, evidence, workflow engine and platform contrasts."

- path: work/theory-writing/fragments/A4_figure_candidates.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Кандидаты на схемы PWG object model, ready/blocked flow, gate lifecycle and PWG-vs-runtime boundaries."

- path: work/theory-writing/fragments/A4_open_questions.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Открытые вопросы и source gaps, especially around adjacent tools and non-canonical cycle 05."

- path: work/theory-writing/fragments/A4_degradation_and_duplication_audit.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Аудит сведения PWG к Beads, issue tracker, durable execution, CRDT editor, worktree or Gas Town."
```

Целевая группа не является ``linked-target-edit``: основной текст один; служебные outputs фиксируют источники, границы, story anchors and risks.
...
### Соседние досье и comparative materials
...
Story-dossiers are secondary: use them only to distinguish PWG from workflow engine, platform agent or local harness.

### Сравнительные отчёты и quality checks
...
Явно известные источники: Beads / Gas Town docs, GitHub Issues dependencies, Linear issue relations, Taskmaster, LangGraph persistence/interrupts, Temporal human-in-the-loop, Pydantic AI durable execution, DBOS, Restate, CodeCRDT, STORM, Anthropic multi-agent research system, Claude Code subagents/worktrees, Codex worktrees, Git worktree, LangChain multi-agent docs, Intermediate Artifacts, AEGIS.

Выводимые источники: inline-ссылки, source registers, tool names, repository links, figure candidates and source notes from `PERSISTENT_WORK_GRAPH_MECHANISM_DOSSIER.md`, `PERSISTENT_WORK_GRAPH_DOSSIER_EXPANSION_REPORT.md`, `GAS_TOWN_METHOD_DOSSIER.md`, `GSD_METHOD_DOSSIER.md`, `BMAD_METHOD_DOSSIER.md` and `PROCESS_METHODS_COMPARATIVE_SYNTHESIS.md`. `cycle_05` may be cited only as rejected/non-canonical caution, never as theoretical basis.
...
Создай первый черновик `work/theory-writing/fragments/A4_persistent_work_graph_boundary.md`. Композиционная функция фрагмента: узел границ PWG как глубокого механизма рабочего состояния. Начинай не с объектной модели, а со сбоя: сессия оборвалась, агент “сделал много”, но непонятно, что завершено, что проверено, что заблокировано, кто владеет продолжением, какие источники уже использованы и где нужно человеческое решение. PWG нужен как ответ на этот сбой, а не как улучшенный task tracker. После этого объясни PWG положительно и отрицательно: что он удерживает и чем не является.
...
Beads/Gas Town, GitHub/Linear, Taskmaster, LangGraph/Temporal, DBOS/Restate, CodeCRDT, STORM, Anthropic multi-agent research открывать только для claims in text.
...
Снова перечитай `work/theory-writing/fragments/A4_persistent_work_graph_boundary.md` и добавь недостающую фактуру по направлению: развести PWG с GitHub/Linear/Taskmaster, durable execution, CRDT, STORM-like mediator and Gas Town as organization.
...
Сделай отдельный failure-pressure pass по `work/theory-writing/fragments/A4_persistent_work_graph_boundary.md`. Проверь, достаточно ли текст показывает сбои, ради которых нужен PWG: ложное завершение, потерянное состояние, дублирование работы, stale graph, read-snapshot staleness, неправильную authority, незакрытые gates и source claims без синтеза.

Если этих сбоев не видно, добавь их как короткое давление на аргумент, а не как список страшилок. Каждый новый внешний источник открывай только под конкретное утверждение и заноси в `work/theory-writing/fragments/A4_source_usage.md`. Не возвращай отвергнутый cycle 05 как основание теории; его можно использовать только как caution.
...
Обязательно открой все новые внешние источники, которые находишь в этом проходе, до того как использовать их. Каждый новый источник занеси в `work/theory-writing/fragments/A4_source_usage.md`. Не добавляй источники для широты и не превращай текст в каталог фактов.
...
Выравняй `work/theory-writing/fragments/A4_persistent_work_graph_boundary.md` как несущий фрагмент общей теории. Проверь, ведёт ли он один продолжающийся смысл, не спорит ли со скелетоном, не пересказывает ли истории и не стал ли он оглавлением будущих глав вместо самостоятельного аргумента.

Сохрани фактуру и ссылки. Убери повторы, слишком локальные рамки и места, где текст выглядит как список тем. Отдельно проверь, не стал ли PWG реализационной схемой или рецептом хранения данных: объектная модель нужна только настолько, насколько она объясняет механизм рабочего состояния. Если часть материала лучше вынести в технический атлас, отметь это в `work/theory-writing/fragments/A4_open_questions.md` или `work/theory-writing/fragments/A4_degradation_and_duplication_audit.md`, но не удаляй важную мысль без замены.
...
Приведи `work/theory-writing/fragments/A4_persistent_work_graph_boundary.md` к русскому техническому языку по языковому протоколу. Не делай стилевую полировку и не меняй аргумент без необходимости.
...
Повторно проверь русский текст `work/theory-writing/fragments/A4_persistent_work_graph_boundary.md`. Ищи гибриды, остаточные английские объяснения, плохие кальки, ошибочно переведённые технические термины и места, где русификация стерла различие между близкими понятиями.
...
Сделай первый стилевой проход по `work/theory-writing/fragments/A4_persistent_work_graph_boundary.md`. Текст должен звучать как человеческий технический аргумент: без summary-интонации, без бюрократических списков, без декоративного пафоса и без размазывания сильных различений.
...
Сделай второй стилевой проход. Проверь, читается ли `work/theory-writing/fragments/A4_persistent_work_graph_boundary.md` как самостоятельный узел теории, а не как досье, оглавление или пересказ материалов. Усиль переходы, если они нужны, но не добавляй новые факты без источников.

Если история используется слишком широко, оставь её как якорь и перенеси ограничение в `work/theory-writing/fragments/A4_story_anchor_map.md`. Если figure-кандидат не поддержан текстом, уточни его в `work/theory-writing/fragments/A4_figure_candidates.md`.
