# Target group plan: C2 — PWG → process profiles

Статус: рабочий план целевой группы. По этому документу можно собрать исполнительный пакет для переходного моста C2. План не является опубликованным текстом теории.

## 1. Обрабатываемые файлы

```yaml
- path: work/theory-writing/fragments/C2_pwg_to_process_profiles.md
  status: future
  role: primary
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Основной мост: почему GSD/BMAD/process profiles are lighter external state forms, not full PWG."

- path: work/theory-writing/fragments/C2_source_usage.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Источники, фактически использованные for PWG-to-process-profiles bridge, including which process-method facts were opened."

- path: work/theory-writing/fragments/C2_story_anchor_map.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Короткие якоря for research-plan, app review, harness and S3-cleanup process-state examples."

- path: work/theory-writing/fragments/C2_figure_candidates.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Кандидаты на иллюстрации for process profile vs PWG: lane, gate, owner, handoff, evidence and cleanup coverage."

- path: work/theory-writing/fragments/C2_open_questions.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Пробелы, если A4, A5 или B2 отсутствуют, либо если деталям источников GSD/BMAD нужен отдельный глубокий проход."

- path: work/theory-writing/fragments/C2_degradation_and_duplication_audit.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Audit that C2 remains a bridge and does not retell A4, A5 or B2."
```

Целевая группа не является ``linked-target-edit``: мост один. Если upstream fragments are missing, the package must read the named dossiers and record the limitation instead of inventing continuity.
...
Если these fragments are absent, read the corresponding dossiers and write the gap to `C2_open_questions.md`.
...
### Дополнительные досье and reports
...
Story-dossier is optional: use Shopify Roast only as a workflow-engine contrast если process profile language starts sliding into executable workflow.
...
Явно известные источники: Open GSD / GSD docs and repositories, `gsd-core`, `gsd-pi`, GSD Auto Mode, BMAD Method docs, BMAD workflow map, BMAD agents, BMAD implementation readiness / checklist materials, Spec Kit workflow docs, Reversa, OpenSpec, AgentSPEX, Gas Town / Beads docs for the upper boundary.

Выводимые источники: inline-ссылки, source registers, external-source lists, figure candidates, tool/repository names and audit notes from `GSD_METHOD_DOSSIER.md`, `BMAD_METHOD_DOSSIER.md`, `SPEC_KIT_METHOD_DOSSIER.md`, `PROCESS_METHODS_COMPARATIVE_SYNTHESIS.md`, `SOURCE_TO_PLAN_MAPPING_AFTER_FULL_REREAD.md` and upstream `A4_source_usage.md`, `A5_source_usage.md`, `B2_source_usage.md` если they exist.
...
Создай первый черновик `work/theory-writing/fragments/C2_pwg_to_process_profiles.md`. Композиционная функция фрагмента: мост от PWG к process profiles. Покажи, что GSD и BMAD могут задавать ритм, роли и фазы, но без долговечного состояния их outputs легко становятся театром процесса. PWG не заменяет GSD/BMAD и не становится methodology brand; он объясняет, какой рабочий субстрат нужен процессным методологиям, чтобы не распасться после сессии.
...
Источники GSD, BMAD, Open GSD и Gas Town открывай только под конкретные утверждения моста.
...
Перечитай `work/theory-writing/fragments/C2_pwg_to_process_profiles.md` и усили его фактурой по направлению: добрать GSD/BMAD examples of state, gates, role outputs and recovery.

Открывай внешние источники только под конкретные утверждения текста. Если добавляешь новый внешний источник, занеси его в `work/theory-writing/fragments/C2_source_usage.md`: источник, почему добавлен, какое утверждение поддерживает, где использован. Ссылки на внутренние досье, планы и отчёты не являются валидными ссылками в основном тексте.
...
Снова перечитай `work/theory-writing/fragments/C2_pwg_to_process_profiles.md` и добавь недостающую фактуру по направлению: связать process-as-artifact with PWG and Gas Town without collapsing them.
...
### P04 — проверка на театр процесса
...
Проверь, не стал ли фрагмент описанием “хороших процессов”. Ему нужно показать сбой “театра процесса”: роль названа, но состояние не сохранено; план создан, но readiness не проверена; handoff объявлен, но нет рабочего объекта; gate есть словами, но не блокирует движение.

Правь мост так, чтобы GSD/BMAD не становились PWG, а PWG не превращался в бренд методологии. Открывай внешние источники только под конкретные утверждения; новые источники заноси в `C2_source_usage.md`.
...
Обязательно открой все новые внешние источники, которые находишь в этом проходе, до того как использовать их. Каждый новый источник занеси в `work/theory-writing/fragments/C2_source_usage.md`. Не добавляй источники для широты и не превращай текст в каталог фактов.
...
Выравняй `work/theory-writing/fragments/C2_pwg_to_process_profiles.md` как несущий фрагмент общей теории. Проверь, ведёт ли он один продолжающийся смысл, не спорит ли со скелетоном, не пересказывает ли истории и не стал ли он оглавлением будущих глав вместо самостоятельного аргумента.

Сохрани фактуру и ссылки. Убери повторы, слишком локальные рамки и места, где текст выглядит как список тем. Если часть материала лучше оформить как самостоятельный материал или раздел концептуально-технического атласа, отметь это в `work/theory-writing/fragments/C2_open_questions.md` или `work/theory-writing/fragments/C2_degradation_and_duplication_audit.md`, но не удаляй важную мысль без замены и не своди материал атласа к несвязному техническому приложению.
...
### P07 — адресное усиление: процесс как последствие для состояния работы
...
Проведи адресный проход по `work/theory-writing/fragments/C2_pwg_to_process_profiles.md`: усили тезис, что процессный элемент полезен только как последствие для состояния работы. Роль, ритуал, фаза, gate, передача работы или восстановление имеют смысл, если они меняют допустимое следующее действие, видимость, владельца, блокировку, проверку или способ продолжения. Если процесс описан как внешний ритуал вокруг агента, перепиши его через последствия для работы.
...
### P08 — адресное усиление: проверка против имитации процесса
...
Проведи адресный проход по `work/theory-writing/fragments/C2_pwg_to_process_profiles.md`: проверь текст против имитации процесса. Процессный профиль не должен быть театральной обвязкой вокруг агента. Если роль, фаза, approval или ceremony не меняют состояние работы, они декоративны. Сделай это центральным различением фрагмента, но не превращай текст в список антипримеров.
...
Оцени текущий текст `work/theory-writing/fragments/C2_pwg_to_process_profiles.md` как готовящийся фрагмент теории: насколько хорошо он выполняет задачу, заданную этим target-group plan, скелетоном и картой документов? Сначала сформулируй проблемы, затем исправь их.
...
Заново оцени уже исправленный `work/theory-writing/fragments/C2_pwg_to_process_profiles.md`. Не считай первый общий редакторский проход достаточным и не повторяй его механически. Прочитай фрагмент как свежий редактор: насколько он теперь выполняет свою задачу в общей теории? Сначала сформулируй проблемы, затем исправь их.
...
Проведи третий общий редакторский проход по `work/theory-writing/fragments/C2_pwg_to_process_profiles.md`. Это не языковая и не стилевая полировка. Задача та же: оценить, насколько фрагмент выполняет поставленную задачу, сначала сформулировать оставшиеся проблемы, затем исправить их.
...
Приведи `work/theory-writing/fragments/C2_pwg_to_process_profiles.md` к русскому техническому языку по языковому протоколу. Не делай стилевую полировку и не меняй аргумент без необходимости.
...
Повторно проверь русский текст `work/theory-writing/fragments/C2_pwg_to_process_profiles.md`. Ищи гибриды, остаточные английские объяснения, плохие кальки, ошибочно переведённые технические термины и места, где русификация стерла различие между близкими понятиями.
...
Сделай первый стилевой проход по `work/theory-writing/fragments/C2_pwg_to_process_profiles.md`. Текст должен звучать как человеческий технический аргумент: без summary-интонации, без бюрократических списков, без декоративного пафоса и без размазывания сильных различений.
...
Сделай второй стилевой проход. Проверь, читается ли `work/theory-writing/fragments/C2_pwg_to_process_profiles.md` как самостоятельный узел теории, а не как досье, оглавление или пересказ материалов. Усиль переходы, если они нужны, но не добавляй новые факты без источников.

Если история используется слишком широко, оставь её как якорь и перенеси ограничение в `work/theory-writing/fragments/C2_story_anchor_map.md`. Для визуального материала не используй правило «кандидат поддерживает аргумент — значит вставить inline» без классификации. Сначала присвой кандидату статус из asset policy: `synthetic_figure`, `local_image_asset`, `external_real_image_candidate` или `editorial_visual_idea`. Inline-вставка допустима только для уже оформленной синтетической схемы, которая прошла usefulness/nontriviality gate, или для разрешённого локального image asset через `<img src="...">`; внешний screenshot/source diagram без asset-pass остаётся в `work/theory-writing/fragments/C2_figure_candidates.md` с причиной и следующим действием. Если кандидат не поддержан текстом, уточни его статус, перенеси в отложенные/отклонённые или сними в `work/theory-writing/fragments/C2_figure_candidates.md`.
