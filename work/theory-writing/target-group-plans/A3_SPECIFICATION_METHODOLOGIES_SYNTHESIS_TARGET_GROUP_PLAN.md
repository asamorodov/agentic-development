# Target group plan: A3 — specification methodologies synthesis

Статус: рабочий план целевой группы. По этому документу можно собрать исполнительный пакет для несущего сравнительного узла A3. План не является опубликованным текстом теории.

## 1. Обрабатываемые файлы

```yaml
- path: work/theory-writing/fragments/A3_specification_methodologies_synthesis.md
  status: future
  role: primary
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Основной фрагмент: сравнительный синтез того, как разные specification methodologies превращают намерение в управляемый артефакт."

- path: work/theory-writing/fragments/A3_source_usage.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Какие внутренние и внешние источники реально использованы для SPDD, Spec Kit, Kiro, TDAD and Constitutional SDD."

- path: work/theory-writing/fragments/A3_story_anchor_map.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Истории-якоря, если они нужны для research/plan, harness, evidence or gates; без пересказа историй."

- path: work/theory-writing/fragments/A3_figure_candidates.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Кандидаты на схемы object-of-control, gates, evidence, lifecycle tail and Handbook/Fieldbook split."

- path: work/theory-writing/fragments/A3_open_questions.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Открытые вопросы, source gaps and uncertain comparisons, especially around two TDADs and Constitutional SDD."

- path: work/theory-writing/fragments/A3_degradation_and_duplication_audit.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Аудит каталогизации, потери protected methodology depth, смешения TDADs and over-duplication with SPDD deep case."
```

Целевая группа не является ``linked-target-edit``: основной текст один; служебные outputs нужны для следа источников и антидеградационной проверки.
...
### Главные методологические досье
...
### Дополнительные внутренние материалы
...
`work/anchor-seed-spdd-old-site.md` and old baseline are anti-degradation references for SPDD depth. They should not make A3 repeat the full SPDD deep case.
...
Story-dossier is optional and should be used only for contract/oracle or migration evidence bridge. A3 is method-synthesis first, story-supported second.

### Сравнительные отчёты и quality audits
...
Явно известные источники: Martin Fowler / Xu Hao `Structured Prompt-Driven Development`, OpenSPDD repository and command templates, REASONS Canvas, GitHub Spec Kit site and repository, Spec Kit command/templates/integrations, Kiro Specs docs, Kiro Steering docs, Constitutional SDD paper and reference repository, TDAD A `Test-Driven AI Agent Definition`, TDAD B `Test-Driven Agentic Development`, TDAD GitHub repositories, SpecSuite-Core.

Выводимые источники: inline-ссылки, source registers, paper titles, repository links, commands, templates, figure candidates and external comparisons from `SPDD_METHOD_DOSSIER.md`, `SPEC_KIT_METHOD_DOSSIER.md`, `KIRO_SPECS_DOSSIER.md`, `TDAD_COMPARATIVE_DOSSIER.md`, `CONSTITUTIONAL_SDD_DOSSIER.md`, `SPECIFICATION_METHODS_COMPARATIVE_SYNTHESIS.md` and dossier quality audits. Открывать внешний источник only when needed for a concrete claim in the fragment.
...
Создай первый черновик `work/theory-writing/fragments/A3_specification_methodologies_synthesis.md`. Композиционная функция фрагмента: сравнительный узел о способах превращать намерение в управляемый specification-layer. Он должен показать не каталог методов, а разные механизмы управления намерением: closed loop, toolkit, IDE surface, tests, constitution/governance. Главная опасность этого фрагмента — превратиться в сравнительную таблицу методологий; удерживай не перечень подходов, а оси различения, по которым видно, как намерение становится управляемым до генерации и проверки.
...
SPDD/OpenSPDD, Spec Kit, Kiro, Constitutional SDD, TDAD papers and repos открывать только под утверждения текста.
...
Перечитай `work/theory-writing/fragments/A3_specification_methodologies_synthesis.md` и усили его фактурой по направлению: добрать детали SPDD, Spec Kit, Kiro и Constitutional SDD, которые меняют сравнение, а не просто украшают обзор.

Открывай внешние источники только под конкретные утверждения текста. Если добавляешь новый внешний источник, занеси его в `work/theory-writing/fragments/A3_source_usage.md`: источник, почему добавлен, какое утверждение поддерживает, где использован. Ссылки на внутренние досье, планы и отчёты не являются валидными ссылками в основном тексте.
...
Снова перечитай `work/theory-writing/fragments/A3_specification_methodologies_synthesis.md` и добавь недостающую фактуру по направлению: добрать две TDAD-линии и развести tests as behavioral specification и tests as regression steering surface.
...
Сделай осевой синтез фрагмента. Не добавляй новые источники без необходимости: сначала проверь, есть ли в тексте ясные оси сравнения specification methodologies — центральный артефакт, место human gate, тип проверки, хвост устаревания и главный failure mode. Если оси не выражены, перестрой фрагмент так, чтобы SPDD, Spec Kit, Kiro, TDAD and Constitutional SDD не шли как последовательный каталог, а раскрывали разные способы стабилизировать намерение.

Сохрани ссылки и фактуру. Если новое утверждение требует внешнего источника, открой первоисточник и занеси его в `work/theory-writing/fragments/A3_source_usage.md`. Внутренние досье не являются валидными ссылками в основном тексте.
...
Обязательно открой все новые внешние источники, которые находишь в этом проходе, до того как использовать их. Каждый новый источник занеси в `work/theory-writing/fragments/A3_source_usage.md`. Не добавляй источники для широты и не превращай текст в каталог фактов.
...
Выравняй `work/theory-writing/fragments/A3_specification_methodologies_synthesis.md` как несущий фрагмент общей теории. Проверь, ведёт ли он один продолжающийся смысл, не спорит ли со скелетоном, не пересказывает ли истории и не стал ли он оглавлением будущих глав вместо самостоятельного аргумента. Отдельно проверь, не превратился ли он в таблицу или каталог методологий: оси сравнения должны вести аргумент, а не заменять его.

Сохрани фактуру и ссылки. Убери повторы, слишком локальные рамки и места, где текст выглядит как список тем. Если часть материала лучше вынести в технический атлас, отметь это в `work/theory-writing/fragments/A3_open_questions.md` или `work/theory-writing/fragments/A3_degradation_and_duplication_audit.md`, но не удаляй важную мысль без замены.
...
Приведи `work/theory-writing/fragments/A3_specification_methodologies_synthesis.md` к русскому техническому языку по языковому протоколу. Не делай стилевую полировку и не меняй аргумент без необходимости.
...
Повторно проверь русский текст `work/theory-writing/fragments/A3_specification_methodologies_synthesis.md`. Ищи гибриды, остаточные английские объяснения, плохие кальки, ошибочно переведённые технические термины и места, где русификация стерла различие между близкими понятиями.
...
Сделай первый стилевой проход по `work/theory-writing/fragments/A3_specification_methodologies_synthesis.md`. Текст должен звучать как человеческий технический аргумент: без summary-интонации, без бюрократических списков, без декоративного пафоса и без размазывания сильных различений.
...
Сделай второй стилевой проход. Проверь, читается ли `work/theory-writing/fragments/A3_specification_methodologies_synthesis.md` как самостоятельный узел теории, а не как досье, оглавление или пересказ материалов. Усиль переходы, если они нужны, но не добавляй новые факты без источников.

Если история используется слишком широко, оставь её как якорь и перенеси ограничение в `work/theory-writing/fragments/A3_story_anchor_map.md`. Если figure-кандидат не поддержан текстом, уточни его в `work/theory-writing/fragments/A3_figure_candidates.md`.
