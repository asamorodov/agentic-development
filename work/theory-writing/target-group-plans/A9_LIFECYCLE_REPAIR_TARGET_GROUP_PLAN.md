# Target group plan: A9 — lifecycle repair

Статус: рабочий план целевой группы. По этому документу можно собрать исполнительный пакет для несущего фрагмента A9. План не является опубликованным текстом теории.

## 1. Обрабатываемые файлы

```yaml
- path: work/theory-writing/fragments/A9_lifecycle_repair.md
  status: future
  role: primary
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Основной фрагмент: repair loop after merge; stale spec/ADR/rules/graph become future agent errors."

- path: work/theory-writing/fragments/A9_source_usage.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Источники, фактически использованные for repair loop, stale ADR/spec/rules/skills/graph and migration evidence claims."

- path: work/theory-writing/fragments/A9_story_anchor_map.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Короткие якоря for documentation/context, skills/rules, Pi harness, research trace and Product Migration repair loop."

- path: work/theory-writing/fragments/A9_figure_candidates.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Кандидаты на иллюстрации for merge -> repair -> future work loop and stale-artifact failure modes."

- path: work/theory-writing/fragments/A9_open_questions.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Gaps around lifecycle-tail evidence, GSD/BMAD cleanup mechanics and skills/rules update sources."

- path: work/theory-writing/fragments/A9_degradation_and_duplication_audit.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Audit that A9 is a repair loop, not an operational cleanup checklist or handbook."
```

Целевая группа не является ``linked-target-edit``: основной текст один. A9 must show the lifecycle tail as a causal loop, not a list of cleanup tasks.
...
Явно известные источники: ADR supersession / stale ADR sources, skills/rules update sources, incident and postmortem sources, SBOM / dependency inventory / supply-chain sources, GSD/BMAD state cleanup sources, migration/runbook/rollback/release-note sources, `AGENTS.md` / rules / skills lifecycle sources, test data drift and architecture-test maintenance sources from source maps.

Выводимые источники: inline-ссылки, source registers, file names, lifecycle terms, migration/runbook examples, release-note details, skills/rules references, architecture/test-maintenance references and figure candidates from `ADR_METHOD_DOSSIER.md`, `PERSISTENT_WORK_GRAPH_MECHANISM_DOSSIER.md`, `GSD_METHOD_DOSSIER.md`, `BMAD_METHOD_DOSSIER.md`, Product Migration dossier/report, Mark/Matt/Armin/Simon stories and `SOURCE_TO_PLAN_MAPPING_AFTER_FULL_REREAD.md`.
...
Создай первый черновик `work/theory-writing/fragments/A9_lifecycle_repair.md`. Композиционная функция фрагмента: узел о том, что merge не завершает жизненный цикл изменения. Он должен собрать хвосты stale specification, ADR, operational projection, rules/skills, work graph and evidence gates into repair loop. Не пиши список “что надо обновить после merge”: покажи, что изменение должно вернуться в систему, которая будет жить после него, иначе следующий агент и следующий человек будут работать по устаревшей карте.
...
ADR lifecycle/supersession, Matt Pocock skills, Product Migration, PWG, GSD/BMAD cleanup and incident/release sources открывать only when used.
...
Перечитай `work/theory-writing/fragments/A9_lifecycle_repair.md` и усили его фактурой по направлению: добрать фактуру по stale ADR/spec/rules/skills and incident/release feedback.

Открывай внешние источники только под конкретные утверждения текста. Если добавляешь новый внешний источник, занеси его в `work/theory-writing/fragments/A9_source_usage.md`: источник, почему добавлен, какое утверждение поддерживает, где использован. Ссылки на внутренние досье, планы и отчёты не являются валидными ссылками в основном тексте.
...
Снова перечитай `work/theory-writing/fragments/A9_lifecycle_repair.md` и добавь недостающую фактуру по направлению: усилить work graph cleanup, migration tail and product migration oracle as repair evidence.
...
Сделай feedback-loop pass. Проверь, что каждый repair target связан с источником обратной связи: release, incident, review, runtime/rollout evidence, migration discovery, support/user signal or failed agent execution. Текст не должен быть энциклопедией stale-артефактов; он должен показать, как lifecycle repair замыкает изменение и возвращает последствия в будущую работу системы.

Исправь места, где repair выглядит как механическое обновление документов. Если добавляешь источник, открой первоисточник и занеси его в `work/theory-writing/fragments/A9_source_usage.md`.
...
Обязательно открой все новые внешние источники, которые находишь в этом проходе, до того как использовать их. Каждый новый источник занеси в `work/theory-writing/fragments/A9_source_usage.md`. Не добавляй источники для широты и не превращай текст в каталог фактов.
...
Выравняй `work/theory-writing/fragments/A9_lifecycle_repair.md` как несущий фрагмент общей теории. Проверь, ведёт ли он один продолжающийся смысл, не спорит ли со скелетоном, не пересказывает ли истории и не стал ли он оглавлением будущих глав вместо самостоятельного аргумента. Отдельно проверь, не стал ли текст списком долгов после merge: он должен показать замыкание repair loop и возвращение последствий изменения в будущую работу системы.

Сохрани фактуру и ссылки. Убери повторы, слишком локальные рамки и места, где текст выглядит как список тем. Если часть материала лучше вынести в технический атлас, отметь это в `work/theory-writing/fragments/A9_open_questions.md` или `work/theory-writing/fragments/A9_degradation_and_duplication_audit.md`, но не удаляй важную мысль без замены.
...
Приведи `work/theory-writing/fragments/A9_lifecycle_repair.md` к русскому техническому языку по языковому протоколу. Не делай стилевую полировку и не меняй аргумент без необходимости.
...
Повторно проверь русский текст `work/theory-writing/fragments/A9_lifecycle_repair.md`. Ищи гибриды, остаточные английские объяснения, плохие кальки, ошибочно переведённые технические термины и места, где русификация стерла различие между близкими понятиями.
...
Сделай первый стилевой проход по `work/theory-writing/fragments/A9_lifecycle_repair.md`. Текст должен звучать как человеческий технический аргумент: без summary-интонации, без бюрократических списков, без декоративного пафоса и без размазывания сильных различений.
...
Сделай второй стилевой проход. Проверь, читается ли `work/theory-writing/fragments/A9_lifecycle_repair.md` как самостоятельный узел теории, а не как досье, оглавление или пересказ материалов. Усиль переходы, если они нужны, но не добавляй новые факты без источников.

Если история используется слишком широко, оставь её как якорь и перенеси ограничение в `work/theory-writing/fragments/A9_story_anchor_map.md`. Если figure-кандидат не поддержан текстом, уточни его в `work/theory-writing/fragments/A9_figure_candidates.md`.
