# Target group plan: C1 — specification → PWG

Статус: рабочий план целевой группы. По этому документу можно собрать исполнительный пакет для переходного моста C1. План не является опубликованным текстом теории.

## 1. Обрабатываемые файлы

```yaml
- path: work/theory-writing/fragments/C1_specification_to_pwg.md
  status: future
  role: primary
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Основной мост: почему спецификация не хранит всю динамику работы and leads to PWG."

- path: work/theory-writing/fragments/C1_source_usage.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Источники, фактически использованные for specification-to-PWG bridge."

- path: work/theory-writing/fragments/C1_story_anchor_map.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Short story anchors for specification pressure, context loss and durable work state."

- path: work/theory-writing/fragments/C1_figure_candidates.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Кандидаты на иллюстрации for spec-to-work-state transition, owner/gate/readiness/recovery."

- path: work/theory-writing/fragments/C1_open_questions.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Открой questions and gaps если required upstream fragments are missing."

- path: work/theory-writing/fragments/C1_degradation_and_duplication_audit.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Audit that C1 remains a bridge and does not retell A2/A3/A4/B2."
```

Целевая группа не является ``linked-target-edit``: мост один. Если upstream fragments are missing, the package should record a gap rather than inventing a final synthesis.
...
Если these fragments are not available, read the corresponding dossiers and record the limitation in `C1_open_questions.md`.
...
Story-dossier is optional and only for migration/oracle pressure если the bridge needs it.
...
Явно известные источники: SPDD/OpenSPDD, Spec Kit, Kiro, TDAD, Constitutional SDD, Beads/PWG adjacent sources из A4 и ADR/contract sources, уже использованные в A2.

Выводимые источники: inline-ссылки, source registers, figure candidates and source notes from A2/A3/A4/B2 source_usage files, specification dossiers and PWG dossier. C1 should normally not discover new sources; it should connect already established materials.
...
Создай первый черновик `work/theory-writing/fragments/C1_specification_to_pwg.md`. Композиционная функция фрагмента: мост между specification/ADR/contract и долговечным рабочим состоянием. Спецификация говорит, что должно измениться и в каких границах; PWG говорит, где живёт сама работа после начала исполнения: кто владеет продолжением, что заблокировано, какие gates открыты, что уже проверено и как восстановиться после обрыва. Не повторяй A2 и A4, покажи именно опасную промежуточную зону между ними.
...
Внешние источники из SPDD, Spec Kit, Kiro и PWG открывай только под конкретные утверждения моста.
...
Перечитай `work/theory-writing/fragments/C1_specification_to_pwg.md` и усили его фактурой по направлению: добрать situations where spec is clear but continuation, ownership or handoff remains unresolved.

Открывай внешние источники только под конкретные утверждения текста. Если добавляешь новый внешний источник, занеси его в `work/theory-writing/fragments/C1_source_usage.md`: источник, почему добавлен, какое утверждение поддерживает, где использован. Ссылки на внутренние досье, планы и отчёты не являются валидными ссылками в основном тексте.
...
Снова перечитай `work/theory-writing/fragments/C1_specification_to_pwg.md` и добавь недостающую фактуру по направлению: связать SPDD/Spec Kit/Kiro with PWG gates/readiness/recovery.
...
### P04 — проход по промежуточному звену
...
Перечитай мост и проверь, раскрыта ли промежуточная зона между спецификацией и PWG: task decomposition, readiness, ownership, gates, handoff, open questions, source/evidence state, recovery point. Это не должен быть повтор A4 и не должен быть новый обзор specification methods. Покажи, в какой момент хорошая спецификация уже есть, но работа ещё не имеет устойчивого состояния.

Открывай внешние источники только под конкретные утверждения. Новые источники заноси в `C1_source_usage.md`; ссылки на внутренние досье не ставь в основной текст.
...
Обязательно открой все новые внешние источники, которые находишь в этом проходе, до того как использовать их. Каждый новый источник занеси в `work/theory-writing/fragments/C1_source_usage.md`. Не добавляй источники для широты и не превращай текст в каталог фактов.
...
Выравняй `work/theory-writing/fragments/C1_specification_to_pwg.md` как несущий фрагмент общей теории. Проверь, ведёт ли он один продолжающийся смысл, не спорит ли со скелетоном, не пересказывает ли истории и не стал ли он оглавлением будущих глав вместо самостоятельного аргумента.

Сохрани фактуру и ссылки. Убери повторы, слишком локальные рамки и места, где текст выглядит как список тем. Если часть материала лучше оформить как самостоятельный материал или раздел концептуально-технического атласа, отметь это в `work/theory-writing/fragments/C1_open_questions.md` или `work/theory-writing/fragments/C1_degradation_and_duplication_audit.md`, но не удаляй важную мысль без замены и не своди материал атласа к несвязному техническому приложению.
...
### P07 — адресное усиление: переход от спецификации к единице работы
...
Проведи адресный проход по `work/theory-writing/fragments/C1_specification_to_pwg.md`: усили механику перехода от спецификации к единице работы. Фрагмент должен показать не то, что specification и PWG оба важны, а что именно переносится в долговечное рабочее состояние: намерение, scope, unresolved questions, acceptance criteria, blockers, ownership, expectations about evidence. Если связь дана общими словами, перепиши её как конкретный переход между артефактами и ответственностями.
...
### P08 — адресное усиление: спецификация не заменяет PWG, PWG не заменяет спецификацию
...
Проведи адресный проход по `work/theory-writing/fragments/C1_specification_to_pwg.md`: проверь границу. Спецификация может описывать желаемое изменение и его смысловые границы, но сама по себе не обязана хранить состояние выполнения. PWG может вести работу, блокировки, владельцев и свидетельства, но не обязан объяснять исходное намерение как specification-layer. Если текст сливает эти функции или делает один слой заменой другого, разведи их.
...
Оцени текущий текст `work/theory-writing/fragments/C1_specification_to_pwg.md` как готовящийся фрагмент теории: насколько хорошо он выполняет задачу, заданную этим target-group plan, скелетоном и картой документов? Сначала сформулируй проблемы, затем исправь их.
...
Заново оцени уже исправленный `work/theory-writing/fragments/C1_specification_to_pwg.md`. Не считай первый общий редакторский проход достаточным и не повторяй его механически. Прочитай фрагмент как свежий редактор: насколько он теперь выполняет свою задачу в общей теории? Сначала сформулируй проблемы, затем исправь их.
...
Проведи третий общий редакторский проход по `work/theory-writing/fragments/C1_specification_to_pwg.md`. Это не языковая и не стилевая полировка. Задача та же: оценить, насколько фрагмент выполняет поставленную задачу, сначала сформулировать оставшиеся проблемы, затем исправить их.
...
Приведи `work/theory-writing/fragments/C1_specification_to_pwg.md` к русскому техническому языку по языковому протоколу. Не делай стилевую полировку и не меняй аргумент без необходимости.
...
Повторно проверь русский текст `work/theory-writing/fragments/C1_specification_to_pwg.md`. Ищи гибриды, остаточные английские объяснения, плохие кальки, ошибочно переведённые технические термины и места, где русификация стерла различие между близкими понятиями.
...
Сделай первый стилевой проход по `work/theory-writing/fragments/C1_specification_to_pwg.md`. Текст должен звучать как человеческий технический аргумент: без summary-интонации, без бюрократических списков, без декоративного пафоса и без размазывания сильных различений.
...
Сделай второй стилевой проход. Проверь, читается ли `work/theory-writing/fragments/C1_specification_to_pwg.md` как самостоятельный узел теории, а не как досье, оглавление или пересказ материалов. Усиль переходы, если они нужны, но не добавляй новые факты без источников.

Если история используется слишком широко, оставь её как якорь и перенеси ограничение в `work/theory-writing/fragments/C1_story_anchor_map.md`. Для визуального материала не используй правило «кандидат поддерживает аргумент — значит вставить inline» без классификации. Сначала присвой кандидату статус из asset policy: `synthetic_figure`, `local_image_asset`, `external_real_image_candidate` или `editorial_visual_idea`. Inline-вставка допустима только для уже оформленной синтетической схемы, которая прошла usefulness/nontriviality gate, или для разрешённого локального image asset через `<img src="...">`; внешний screenshot/source diagram без asset-pass остаётся в `work/theory-writing/fragments/C1_figure_candidates.md` с причиной и следующим действием. Если кандидат не поддержан текстом, уточни его статус, перенеси в отложенные/отклонённые или сними в `work/theory-writing/fragments/C1_figure_candidates.md`.
