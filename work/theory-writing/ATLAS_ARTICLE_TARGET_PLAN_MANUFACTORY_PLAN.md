...
- dossier image-candidate sections используются как обязательный источник визуальных решений: кандидаты из разделов `Кандидаты на иллюстрации`/`image candidates` получают disposition, а релевантные внешние кандидаты ставятся inline как `<figure>` и зеркалятся в нижнем разделе `Внешние изображения для asset-pass` и external image queue;
...
### Dossier-backed completeness requirement for generated plans

Every generated atlas article target-group plan must include a strong dossier-backed completeness rule. The plan must make clear that the dossier is the primary source base, not optional inspiration: all dossier/source facts relevant to the article's reader question and purpose should be transferred into the future article or explicitly dispositioned. Generated plans must include coverage/disposition requirements in P03, transfer-audit requirements in source-depth passes, free material-expansion passes focused on factual/conceptual underdevelopment, and final verification that blocks silent loss of relevant dossier material.
