# P13 — перепись естественным русским языком

Ниже — цельная переписанная версия главы после активного добора P11 и визуального прохода P12. Я не ограничивался заменой отдельных слов: заново собраны переходы между разделами, убран лишний англо-русский клей, уточнены claim/gate/source state/prime и добавлены две синтетические table-figures, которые помогают отличить summary от состояния работы and signal от приказа.
...
Агент действительно мог закрыть локальный кусок. Он обновил DTO, добавил mapping, написал unit tests, получил зелёный unit run. Возможно, сделал commit and push. На уровне своей сессии он не обязательно ошибся. Проблема возникает тогда, когда система принимает локальный конец операции за конец изменения.
...
Важна не конкретная команда. Важна смена формы: готовность перестаёт быть пожеланием и становится запросом к состоянию графа.
...
Temporal and LangGraph show the runtime side of the same pressure. Workflow can pause for approval or interrupt graph execution and resume later. But gate in PWG answers a different question: which work item is blocked, what condition resolves it, who owns the decision, what checking basis is required, and what state changes when it resolves.
...
## Среда выполнения и PWG — разные слои
...
## Остаточные места для следующего редакторского прохода

1. В этой версии улучшена русская связность, но часть source-native формулировок оставлена сознательно. Следующий проход может точечно решить, где `runtime`, `source state`, `restoration packet`, `ready`, `claim`, `gate` нужно оставить как технические термины, а где заменить на русские пояснения.
2. Разделы «Вывод subagent должен вернуться в граф», «Среда выполнения и PWG — разные слои», «Как это выглядит в текущей практике» ещё частично говорят на английском из-за плотности ссылок и source-native terms. Их можно дополнительно разгладить, не теряя фактуру.
3. Таблицы `fig-vii-summary-vs-work-graph` and `fig-vii-signal-triage` стоит проверить на совместимость с CSS сайта: если таблицы плохо ведут себя на узкой ширине, заменить на list-card layout.
4. Слово «свидетельство» в основной главе практически не используется как центральный термин; оставлены «проверочные основания», «проверочные сигналы», «основания принятия».
