...
| Ex-A3_agent_run_to_accepted_change_theory_fragment.md | XI, XII, XIII | theory-ready donor / not Atlas article | agent run ≠ accepted change; trace/check/review/decision status ladder; accepted/rejected/needs-changes/superseded/unknown; post-merge route | Хороший текст, но был написан в неверном разрезе для Атласа; при переносе не сохранять статью целиком, а разложить по XI/XII/XIII. | Должен усиливать Part V skeleton, но не становиться отдельной главой и не заменять технический Атлас A6/A7. |
...
## Harness retrofit coordination

После ADR-0018 фрагментно-главная маршрутизация должна учитывать не только перенос A/B/C/ex-A3-фрагментов, но и ретроспективный harness-retrofit уже написанных глав I–X. Не создавать отдельный parallel patch map: такие изменения должны фиксироваться в `THEORY_CHAPTER_ATTACHMENT_MAP.md`, потому что только она одновременно видит Atlas layers, ex-A3, фрагменты, stories and future source attachments.

Главное ограничение: harness-frame не заменяет A/B/C/ex-A3-фрагменты, не создаёт новую главу и не превращает Теорию в Атлас. При будущей ретроспективной правке сначала нужен точный patch list and human checkpoint, затем минимальные изменения основного текста и companion-файлов.
