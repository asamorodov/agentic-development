...
Kiro Specs лучше читать как productized spec discipline: IDE/Web/CLI поверхность пытается сделать specs не внешней практикой, а встроенным состоянием продукта. Ядро — Feature Specs с последовательностью `requirements.md → design.md → tasks.md → execution`; рядом находятся Bugfix Specs, Quick Plan и Analyze Requirements. Source-opening pass добавляет ещё два важных слоя: проверку требований через нейросимвольный analysis и проверку реализации через property-based tests. Поэтому Kiro находится не просто рядом со Spec Kit, а показывает другую траекторию: spec-first workflow становится продуктовой средой, где требования, анализ, тесты, steering, hooks, web autonomy и PR delivery постепенно собираются в одну платформу.
...
Source-opening уточнение: deep requirements analysis у Kiro устроен не как “LLM перечитал требования и дал советы”. Публичный deep-dive описывает три стадии: refinement делает thesis-level acceptance criteria testable and solution-free, auto-formalization переводит требования в formal model, а logical analysis ищет contradictions, gaps и accepted/rejected scenarios [Kiro deep requirements analysis](https://kiro.dev/blog/deep-spec-analysis/). Refinement отдельно проверяет EARS-pattern misuse, vague qualifiers и implementation-level language. Это важная фактура: Analyze Requirements — не просто расширенный review, а попытка превратить requirements в материал, пригодный для формального анализа.

Блог “Specs just got faster” формулирует пользовательскую поверхность проще: после генерации requirements можно выбрать `Analyze Requirements`; Kiro ищет ambiguity через несколько интерпретаций требований и uses automated reasoning to validate consistency and gaps [Specs just got faster](https://kiro.dev/blog/faster-smarter-specs/). Для досье это усиливает distinction между Feature Specs как authoring flow и Analyze Requirements как correctness / ambiguity layer поверх требований.
...
Source-opening уточнение: Kiro также выносит “spec correctness” в property-based testing. Документация описывает этот слой как перевод natural language specifications в executable properties и generation of comprehensive test cases; цель — проверить универсальные свойства на input spaces, а не только отдельные example-based unit tests [Correctness with Property-based tests](https://kiro.dev/docs/specs/correctness/). При этом Kiro честно ограничивает claim: PBT не гарантирует отсутствие всех багов, но даёт более сильное evidence of correctness, чем отдельные examples. Для будущей теории это важный мост между specification artifacts и verification harness.
...
Отдельно нужно держать Web autonomous mode. В Kiro Web Preview autonomous mode агент сначала задаёт clarification questions, потом строит plan, delegates work to specialized sub-agents and opens a pull request when complete [Autonomous mode](https://kiro.dev/docs/web/autonomous-mode/). Это не равно Feature Specs в IDE, но показывает направление жизненного цикла: спецификация и планирование становятся входом для более автономного delivery loop. В будущей теории это может связать Kiro Specs с темой “от спецификации к PR”, но досье не должно подменять specs-раздел рассказом о Web autonomy.
...
Source-opening pass уточнил риск “product confidence”. Нейросимвольный requirements analysis звучит сильнее обычного LLM review, но его результат всё равно зависит от того, насколько требования можно формализовать. Если acceptance criteria остаются слишком thesis-level, refinement сначала должен довести их до testable events/inputs/states/outputs; без этого formalization itself получает плохой вход [Kiro deep requirements analysis](https://kiro.dev/blog/deep-spec-analysis/). Поэтому Kiro Specs не отменяет ремесло формулирования требований, а частично автоматизирует обнаружение дыр после того, как требования уже достаточно структурированы.

Второй риск — смешать разные product surfaces. IDE Feature Specs, Analyze Requirements, Correctness/PBT и Web autonomous mode относятся к одной Kiro-экосистеме, но не являются одним и тем же workflow. Для теории нужно использовать их как связанные слои, а не как доказательство, что любой Kiro spec автоматически проходит полный путь от требований до PR.
...
- `deep-spec-analysis` — раскрыт как источник о refinement → auto-formalization → logical analysis и проверке ambiguous/gap/contradictory requirements.
- `faster-smarter-specs` — добавлен как продуктовая формулировка Analyze Requirements, Quick Plan и parallel task execution.
- `docs/specs/correctness` — раскрыт как PBT-layer: natural language specs → executable properties → test cases.
- `docs/web/autonomous-mode` — добавлен как соседний lifecycle-layer: clarification, planning, specialized sub-agents, PR.
