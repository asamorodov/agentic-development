...
BMAD Method лучше читать не как «набор промптов для агентов», а как попытку заменить длинный хрупкий разговор с coding agent цепочкой файловых контрактов. В базовой схеме метод делает четыре вещи:

- устанавливает в проект `_bmad/` и `_bmad-output/`, то есть отделяет конфигурацию/skills/рабочие процессы от создаваемых планировочных и реализационных артефактов;
- заставляет выбрать масштаб работы: `Quick Flow`, полный `BMad Method` или `Enterprise`, причём выбор должен зависеть от риска, неопределённости и числа затронутых систем, а не только от формального числа stories;
- наращивает контекст через документы: brief/research → PRD → UX/architecture → epics/stories → `sprint-status.yaml` → implementation/review/retro;
- перед реализацией снова сужает контекст: story file, `project-context.md`, sprint state, previous-story intelligence и review trail должны дать dev-agent достаточно конкретную рамку без загрузки всего предыдущего обсуждения.

Сильная идея BMAD не в том, что «документов должно быть больше», а в том, что контекст, решения, статус и проверка работы становятся проверяемыми файлами. Эти файлы можно перечитать в свежем чате, передать другому agent/persona, положить в Git и использовать как след для ревью.
...
### Phase 1: Analysis / анализ, необязательная фаза
...
- `bmad-domain-research`, `bmad-market-research`, `bmad-technical-research` - проверка domain, market или technical assumptions, результат research наблюдениями.
...
### Phase 2: Planning / планирование, обязательная фаза
...
### Phase 3: Solutioning / проектное решение
...
Workflow Map добавляет дополнительные рабочие процессы реализации: `bmad-correct-course` для существенных изменений посреди sprint, `bmad-sprint-status` для обновления sprint status и `bmad-investigate` для forensic case investigation с результатом `{slug}-investigation.md`. Источник: https://raw.githubusercontent.com/bmad-code-org/BMAD-METHOD/main/docs/reference/workflow-map.md
...
```text
brainstorming-report.md / research наблюдениями / product-brief.md / prfaq-{project}.md
...
- Runtime/config слой в `_bmad/`, включая конфигурацию модулей и настройку skills.
...
Full Project Scan использует `documentation-requirements.csv` как scan guide для 12 project types: web, mobile, backend, cli, library, desktop, game, data, extension, infra, embedded. Scan level выбирается человеком: Quick Scan читает в основном конфиги, manifests и структуру каталогов; Deep Scan читает critical directories; Exhaustive Scan читает все source files, кроме типичных сгенерированных и тяжёлых каталогов. Файл состояния обновляется после каждого шага с timestamp, current_step, completed_steps, outputs_generated и краткими наблюдениями; workflow явно требует очищать подробные наблюдениями из контекста после записи документов, чтобы не держать весь codebase в разговоре. Источник: https://raw.githubusercontent.com/bmad-code-org/BMAD-METHOD/main/src/bmm-skills/1-analysis/bmad-document-project/workflows/full-scan-instructions.md
...
Новый проход раскрыл сам `documentation-requirements.csv`, а не только checklist. Файл действительно является компактной таблицей на 12 строк project types и 24 колонки: `project_type_id`, boolean-флаги вроде `requires_api_scan`, `requires_data_models`, `requires_state_management`, `requires_ui_components`, затем группы паттернов (`key_file_patterns`, `critical_directories`, `integration_scan_patterns`, `test_file_patterns`, `auth_security_patterns`, `schema_migration_patterns`, `ci_cd_patterns`, `asset_patterns`, `protocol_schema_patterns` и другие). Это закрывает прежний открытый вопрос о том, существует ли таблица только как пересказ в workflow docs: она читается напрямую и показывает data-driven характер `bmad-document-project`. Источник: https://raw.githubusercontent.com/bmad-code-org/BMAD-METHOD/main/src/bmm-skills/1-analysis/bmad-document-project/documentation-requirements.csv

Deep-dive-режим оказался жёстче обычного scan. `deep-dive-instructions.md` прямо запрещает sampling/guessing/relying solely on tooling output и требует literal full-file review выбранной области; пользователь сначала подтверждает target, тип, путь и estimated files, затем workflow читает каждый файл в этой области. Для теории это важное уточнение: BMAD различает быстрый обзор проекта, глубокое документирование brownfield-кода и точечный exhaustive deep-dive перед risky feature work. Источник: https://raw.githubusercontent.com/bmad-code-org/BMAD-METHOD/main/src/bmm-skills/1-analysis/bmad-document-project/workflows/deep-dive-instructions.md

## Что в BMAD является ядром, а что расширением

Для будущего текста важно не смешивать несколько уровней. Ядро этого досье — BMM как модуль Agile/SDLC-работы: установка, файловые артефакты, рабочие процессы анализа, планирования, архитектуры, stories, реализации, ревью и статуса sprint. BMad Ecosystem шире: official modules вроде TEA, BMad Builder, Creative Intelligence Suite и Game Dev Studio могут добавлять собственных агентов, tasks и рабочие процессы поверх базовой рамки. Их полезно описывать как расширения той же установочной и агентной модели, но не как обязательные шаги каждого BMAD-проекта.

Отсюда редакционное правило: TEA и Enterprise-ветку можно использовать для объяснения предельного варианта BMAD с evidence lifecycle, NFR, release gate и compliance archive, но основной рассказ о BMAD должен сначала показать базовую цепочку документов и stories. Иначе читатель решит, что BMAD всегда означает тяжёлый regulated-process, хотя источники одновременно описывают Quick Flow, полный Method track и отдельные расширения.
...
- Релиз v6.8.0 добавляет ещё один важный поворот: `bmad-spec` становится core skill, вытесняя прежний `bmad-distillator`, а `bmad-ux` заменяет прежнюю UX-связку двумя файлами `DESIGN.md` и `EXPERIENCE.md`; handoff из design в engineering описан как sealed file contract. Там же указаны web bundles для Gemini Gems и ChatGPT Custom GPTs, `bmad-automator` на `next` channel и community-alpha `bmad-method-ui`. Для досье это не отдельный BMAD workflow, а сигнал быстрой эволюции поверхности и перехода к более формализованным file contracts. Источник: https://github.com/bmad-code-org/BMAD-METHOD/releases
...
- Если проблема требует расследования до планирования исправления, `bmad-investigate` создаёт или продолжает case file, отделяет confirmed наблюдениями от deductions/hypotheses, реконструирует timeline, фиксирует missing evidence и завершает работу понятной передачей. Финальный outcome содержит confidence: High для confirmed root cause with deterministic repro, Medium для deduction with minor uncertainty, Low для hypothesis with clear data gap. Источник: https://raw.githubusercontent.com/bmad-code-org/BMAD-METHOD/main/src/bmm-skills/4-implementation/bmad-investigate/SKILL.md
...
- `project-context.md` является living document. Если architecture decisions или conventions меняются, а файл не обновляется, рабочие процессы реализации будут продолжать тянуть старые правила. Источник: https://raw.githubusercontent.com/bmad-code-org/BMAD-METHOD/main/docs/explanation/project-context.md
...
Эти заметки оставлены как исторический хвост предыдущих проходов. Они не являются основной структурой source dossier и не должны автоматически маршрутизировать материал в будущие документы. При сборке теории их можно использовать только как вспомогательные наблюдения после повторного просмотра фактуры выше.
...
- `documentation-requirements.csv` raw: https://raw.githubusercontent.com/bmad-code-org/BMAD-METHOD/main/src/bmm-skills/1-analysis/bmad-document-project/documentation-requirements.csv  
  Роль: прямой источник по 12 project types и 24-column schema для `bmad-document-project`; закрывает прежний вопрос о полной таблице project type flags and scan patterns.
- `bmad-document-project` deep-dive instructions raw: https://raw.githubusercontent.com/bmad-code-org/BMAD-METHOD/main/src/bmm-skills/1-analysis/bmad-document-project/workflows/deep-dive-instructions.md  
  Роль: граница deep-dive режима: literal full-file review выбранной области, без sampling/guessing.
- `bmad-document-project` full scan workflow wrapper: https://raw.githubusercontent.com/bmad-code-org/BMAD-METHOD/main/src/bmm-skills/1-analysis/bmad-document-project/workflows/full-scan-workflow.md  
  Роль: показывает, что full scan является отдельным sub-workflow с config loading, `communication_language`, `document_output_language` и user-input gates.
- External evaluation: “I Tested Three Spec-Driven AI Tools. Here’s My Honest Take”: https://ranthebuilder.cloud/blog/i-tested-three-spec-driven-ai-tools-here-s-my-honest-take/  
  Роль: не primary, но конкретный comparative experience report по BMAD Full / BMAD Quick / Spec-Kit / OpenSpec на одной feature; полезен для рисков time/cost/reviewability/parallel work. Ограничение: авторская оценка, не BMAD-документация.
- Reddit thread “Burning too many tokens with BMAD full flow”: https://www.reddit.com/r/ClaudeCode/comments/1rurgdd/burning_too_many_tokens_with_bmad_full_flow/  
  Роль: слабый community сигнал по token overhead полного BMAD flow; использовать только как anecdotal risk signal, не как статистику.
- GitHub issue #511 “big tokens cost”: https://github.com/bmad-code-org/BMAD-METHOD/issues/511  
  Роль: issue-level сигнал о повторном чтении файлов и token cost в `/dev`; слабый источник, но полезен для failure modes.
- GitHub issue #813 про `document-project` path/reference failure: https://github.com/bmad-code-org/BMAD-METHOD/issues/813  
  Роль: пример cross-tool/path fragility в v6 document-project; использовать осторожно, потому что issue относится к конкретной версии/среде.
- Atlassian community thread про Rovo Dev support: https://community.developer.atlassian.com/t/rovo-dev-support-in-bmad-method/99157  
  Роль: слабый, но показательный источник по трудности support/matrix для агентных IDE: maintainer пишет, что Rovo Dev support хочется поддерживать, но core team не использует его активно.
- Secondary comparison BMAD / Claude Flow / Gas Town: https://re-cinq.com/blog/multi-agent-orchestration-bmad-claude-flow-gastown  
  Роль: внешний framing для trade-off “documentation vs memory vs git persistence”; не использовать как primary источник деталей BMAD.
...
- Investigation evidence ladder.  
  Что изображено: raw evidence -> Confirmed наблюдениями -> Deduced наблюдениями -> Hypothesized branches -> conclusion confidence -> route to quick-dev/correct-course/create-story/code-review.  
...
- BMAD cost/depth trade-off diagram.  
  Что изображено: `BMAD Full` как глубокий planning/evidence path, `BMAD Quick` как компактный spec/implementation path, рядом внешние риски reviewability/time/cost.  
  Зачем пригодится: показать, что BMAD не должен применяться одинаково к любой задаче.  
  Где использовать: теория или Fieldbook.  
  Статус: создать собственную схему; external evaluation использовать только как один опыт, не как норму.
- BMAD vs Gas Town persistence diagram.  
  ...
  Зачем пригодится: уточнить локальное различение “документальный handoff” против “durable task/orchestration layer”.  
  Где использовать: Cross-Story / теория.  
  Статус: candidate; не рисовать без сверки с Gas Town dossier.
...
- Насколько часто реальные команды используют полный Method track, а не Quick Flow или внешние automators? Этот проход нашёл несколько внешних сигналов, но они остаются anecdotal / comparative, а не статистикой. Нужен отдельный проход по experience reports, если BMAD станет крупной частью theory.
- Насколько community experience reports подтверждают устойчивую стратегию “BMAD Full for planning, lean/quick flow for execution”? Пока это видно как гипотеза из Reddit/external evaluation, но не как доказанная практика.
- Вопрос о прямом доступе к `documentation-requirements.csv` в этом проходе закрыт: raw file прочитан, таблица содержит 12 project types и 24-column schema. Остаётся не вопрос доступа, а вопрос, насколько подробно эту таблицу переносить в финальную теорию: вероятно, достаточно показать 3–4 флага и сам принцип data-driven scan guide.
