...
Source-opening pass добавил Foundry как более конкретный constitutional artifact. `constitution.md` в Cisco Foundry seed содержит не абстрактные лозунги, а operational principles с rationale and governance: `Evidence Over Assertion`, `Surface Only What Survives`, `Liveness By Heartbeat, Never By Clock`, `Claims Are Atomic And Mortal`, `The Provider Is The Rate Arbiter`, `Coverage Before Yield`, `Exploited Means Demonstrated`, `Sandbox By Infrastructure, Not By Prompt`, `The Operator Outranks Every Agent`, `Persist Atomically` [Foundry constitution](https://raw.githubusercontent.com/CiscoDevNet/foundry-security-spec/main/constitution.md). Это существенно расширяет материал CSDD: constitution может задавать не только secure coding constraints, но и правила agent fleet / verification economy.

Foundry constitution also defines precedence: if constitution conflicts with `spec.md`, the constitution wins; if it conflicts with generated `plan.md` or `tasks.md`, the generated artifact is wrong and `/speckit.analyze` should flag it [Foundry constitution](https://raw.githubusercontent.com/CiscoDevNet/foundry-security-spec/main/constitution.md). Это важный механизм, потому что “constitution” становится не advice, а более высокий нормативный слой в цепочке artifact generation.
...
Foundry `spec.md` показывает, как constitutional constraints превращаются в проверяемые user stories, functional requirements и acceptance checks. Например, spec содержит user stories для reviewer/operator/developer roles, независимые acceptance checks и edge cases вроде killed agent mid-claim, simultaneous claims, duplicate detection, heartbeat stale ownership release [Foundry spec](https://raw.githubusercontent.com/CiscoDevNet/foundry-security-spec/main/spec.md). Для будущей теории это даёт хороший пример: constitution не живёт отдельно; она связывается с requirements and scenarios, которые потом могут проверяться в planning/analyze loop.

Project CodeGuard полезно держать как соседний, но не тождественный источник: он описывает себя как open-source, model-agnostic security framework, который embeds secure-by-default practices into AI coding agent workflows [Project CodeGuard](https://github.com/cosai-oasis/project-codeguard). Его лучше использовать для расширения темы security guardrails, но не как доказательство именно Constitutional SDD-method, если конкретный workflow не совпадает с paper / Spec Kit constitution path.
...
Source-opening pass усилил скептическую оговорку. Paper называет снижение defects by 73% compared to unconstrained AI generation, но это заявлено в рамках banking microservices case study and reference implementation, а не как общая доказанная константа для всех проектов [arXiv:2602.02584](https://arxiv.org/abs/2602.02584). В будущей теории это число можно использовать только с условиями: domain, case-study setup, constitutional constraints, compliance traceability matrix and comparison baseline.

Foundry также показывает риск “constitution overreach”: конституция может начать регулировать runtime/fleet behavior, security evidence, triage economics и persistence semantics. Это ценно, но в теории нужно явно отделять secure-code constitution, agent-fleet constitution и organizational governance constitution. Иначе CSDD станет слишком широким зонтиком и потеряет точность.
...
Constitutional SDD нужно читать не как ещё одну полную замену SDLC, а как security/governance overlay поверх spec-driven workflow. Его центр — versioned constitution: machine-readable / reviewable artifact, который должен ограничивать specs, plans и tasks раньше, чем агент начнёт писать код. В этом проходе особенно важно не раздувать статус материала: paper и banking case study дают сильный исследовательский пример, Foundry seed даёт очень конкретный конституционный artifact, а Project CodeGuard — соседний набор secure-by-default правил для AI coding agents. Это разные уровни, и их нельзя смешивать в один “зрелый стандарт”.

## Источники

### Дополнительный источникный срез chat source-pass

- `arXiv:2602.02584` — зафиксирован как исследовательский источник с важной, но ограниченной метрикой 73% defect reduction in banking case study.
- Cisco Foundry `constitution.md` — раскрыт как concrete constitution с operational principles, precedence, governance and compliance review.
- Cisco Foundry `spec.md` — раскрыт как пример связи constitution → user stories / FR / acceptance checks / edge cases.
- Project CodeGuard — добавлен как соседний security-guardrail corpus, не равный CSDD, но полезный для контекста secure-by-default agent workflows.
