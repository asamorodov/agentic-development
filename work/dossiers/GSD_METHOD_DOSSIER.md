...
Source-opening уточнение: current `gsd-core` docs also show a strong token-economy design move. v1.40 ships six namespace meta-skills (`/gsd-workflow`, `/gsd-project`, `/gsd-quality`, `/gsd-context`, `/gsd-manage`, `/gsd-ideate`) so the system prompt can expose about 120 tokens of routers instead of a flat list of 86 skills at about 2,150 tokens; concrete commands remain directly callable [GSD User Guide](https://www.opengsd.net/docs/v1/user-guide). This is not just UI tidiness. It is a methodological answer to the same problem we saw in многопроходный документный процесс: tool/process surfaces become too large for cheap prompt exposure, so they need hierarchical routing.
...
Source-opening уточнение по `gsd-core` configuration: `.planning/config.json` now exposes many workflow switches that materially change the method, not just preferences. The schema includes `workflow.nyquist_validation`, `workflow.human_verify_mode`, `workflow.plan_bounce`, `workflow.code_review`, `workflow.security_enforcement`, `parallelization.max_concurrent_agents`, `git.branching_strategy`, `gates.*`, `agent_skills`, `response_language`, `global_learnings`, and `review.models.<cli>` for routing review to tools such as Codex/Gemini/OpenCode [GSD Configuration](https://www.opengsd.net/docs/v1/configuration). For theory this matters: GSD is a configurable process machine; two teams saying “we use GSD” may actually run different verification, review, parallelization and safety policies.

Source-opening pass found a concrete timeout/supervision mechanism highly relevant to our own loop design. GSD command docs mention checkpoint heartbeats for background `execute-phase` runs: they emit `[checkpoint]` markers at every wave and plan boundary so the Claude API SSE stream does not idle long enough to trigger `Stream idle timeout - partial response received` on multi-plan phases [GSD Commands](https://www.opengsd.net/docs/v1/commands). This is a useful pattern for многопроходный документный процесс: long work should not only write files; it should also produce cheap liveness/progress signals at semantic boundaries.
...
Source-opening pass added the adaptive context detail. During `execute-phase`, plans are grouped into dependency waves; each executor gets a fresh 200K context window or up to 1M for models that support it. If context window is 500K+ tokens, GSD enriches subagent prompts with prior wave summaries and phase context; for standard 200K windows, prompts use truncated versions with cache-friendly ordering [GSD Core Architecture](https://www.opengsd.net/docs/v1/architecture). This is a concrete example of context budget as a runtime-dependent policy, not a fixed textual rule.
...
Source-opening уточнение по validation: User Guide describes Nyquist validation as pre-execution mapping of automated test coverage to each phase requirement. Researcher detects test infrastructure, maps requirements to test commands and creates Wave 0 tasks for needed test scaffolding; plan-checker enforces this as an 8th verification dimension, rejecting plans without automated verify commands [GSD User Guide](https://www.opengsd.net/docs/v1/user-guide). There is also `/gsd-validate-phase` for retroactive validation: it detects whether `VALIDATION.md` and `SUMMARY.md` exist, discovers implementation, maps requirements to tests, presents gap plan, spawns an auditor and updates `VALIDATION.md` [GSD User Guide](https://www.opengsd.net/docs/v1/user-guide).

Architecture docs add codebase drift gate after execution. It compares changes since `last_mapped_commit` against `.planning/codebase/STRUCTURE.md`, counts structural changes such as new directories, barrel exports, migrations and route modules, then warns or auto-remaps when threshold is reached [GSD Core Architecture](https://www.opengsd.net/docs/v1/architecture). This is important for future theory: verification is not only “tests pass”, but also “the codebase map did not silently become stale”.
...
Source-opening pass sharpens a configuration-risk point. Because GSD exposes review models, parallelization, gates, git strategy, security enforcement, Nyquist validation and context/window behavior as configuration, the method is powerful but also hard to compare from outside. A case study must record `config.json`-level settings, otherwise “GSD succeeded/failed” is under-specified. This is especially true for `parallelization.max_concurrent_agents`, `workflow.nyquist_validation`, `workflow.code_review`, `workflow.security_enforcement`, and review model routing [GSD Configuration](https://www.opengsd.net/docs/v1/configuration).

The checkpoint heartbeat pattern also creates a lesson: long-running orchestration can fail for transport/liveness reasons even when the underlying plan is healthy. A robust agentic SDLC needs semantic progress heartbeats, resumable state and file-backed artifacts; otherwise a timeout looks like methodological failure when it is really supervision failure [GSD Commands](https://www.opengsd.net/docs/v1/commands).
...
- `GSD User Guide` — раскрыты namespace routers, full lifecycle walkthrough, Nyquist validation, retroactive validation and workflow diagrams.
- `GSD Configuration` — добавлен как источник config-level differences: workflow toggles, gates, parallelization, review routing, security enforcement.
- `GSD Commands` — раскрыт checkpoint heartbeat pattern for long background runs and timeout prevention.
- `GSD Core Architecture` — раскрыты wave execution, adaptive context enrichment, parallel commit safety and codebase drift gate.
