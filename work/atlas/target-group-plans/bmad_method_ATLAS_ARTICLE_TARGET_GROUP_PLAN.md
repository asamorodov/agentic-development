...
Вопрос читателя: как BMAD распределяет агентную разработку между ролями, документами, gates, workflows and story context так, чтобы работа переходила через читаемые, проверяемые и исправляемые артефакты?

Центральная ось статьи: BMAD важен как artifact-first SDLC profile: он не просто задаёт personas and commands, а строит передачу контекста через PRD, architecture, stories, sprint state, review, correct-course and brownfield documentation so that agents work inside artifacts humans can inspect and repair.

Соседние границы: BMAD ≠ GSD: artifact-first SDLC profile vs runtime/process supervision. BMAD ≠ PWG: documents/roles/workflows vs durable graph state. BMAD ≠ Gas Town: project methodology vs operating environment. BMAD ≠ Spec Kit/Kiro/SPDD/TDAD: full SDLC profile vs specification/test methods. ADR stores decisions, not the whole execution method.

Визуальный слой: Приоритетны real workflow maps, role/artifact diagrams, file tree `_bmad/` + `_bmad-output/`, sprint-status state machine, brownfield scan/document-project flow, checkpoint preview/correct-course surfaces, TEA/release-gate visuals. Synthetic diagrams only for artifact handoff or BMAD/GSD/PWG/Gas Town boundaries.
...
Статья не должна стать каталогом personas/commands, ecosystem overview or generic SDLC guide. Каждая деталь BMAD должна показывать функцию в lifecycle: creates next input, narrows implementation context, records state, gives human decision point, produces evidence or repair path.

## 2.1. Фактура без coverage-бюрократии

BMAD-статья опирается на досье и первоисточники, но package не должен снова превращать перенос фактуры в coverage/disposition-бюрократию. Жёсткий режим `dossier-backed completeness / relevant but untransferred` считается откатанным: не нужно строить тотальную matrix по всему досье и доказывать статус каждого релевантного фрагмента.

Рабочий ориентир проще: ключевые тезисы статьи должны иметь достаточные технические опоры. Для этой статьи особенно важны: BMAD roles, agents, files, commands, handoffs, planning/implementation transitions and failure modes. Если без такой детали раздел остаётся общей прозой, материал лучше встроить в основной текст. Если деталь не помогает reader question, относится к соседней статье или ломает последовательность, её не нужно вставлять ради отчётности.
...
1. **Why BMAD: fragile agent work loses requirements/context/decisions.**
2. **Quick Flow, full BMad Method, Enterprise as different lifecycle depths.**
3. **File frame: `_bmad/`, `_bmad-output/`, manifest/config, planning and implementation artifacts.**
4. **Phases: analysis, planning, UX/design/architecture, implementation, review/retro/correct-course.**
5. **Story as narrow implementation context and sprint state.**
6. **Brownfield/document-project as project knowledge for future agents.**
7. **Quality/review/repair: validation, checkpoint preview, investigation, correct-course, TEA/release gate.**
8. **Applicability and over-ceremony.**
...
```yaml
- path: work/atlas/articles/bmad_method.md
  status: future
  role: primary
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the BMAD atlas article."
- path: work/atlas/articles/bmad_method_source_usage.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the BMAD atlas article."
- path: work/atlas/articles/bmad_method_source_transfer_ledger.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the BMAD atlas article."
- path: work/atlas/articles/bmad_method_image_plan.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the BMAD atlas article."
- path: work/atlas/articles/bmad_method_external_image_queue.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the BMAD atlas article."
- path: work/atlas/articles/bmad_method_open_questions.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the BMAD atlas article."
- path: work/atlas/articles/bmad_method_theory_links.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the BMAD atlas article."
- path: work/atlas/articles/bmad_method_degradation_and_duplication_audit.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the BMAD atlas article."
- path: work/atlas/articles/bmad_method_readiness_report.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
...
content/assets/theory-images/fowler-harness-overview.png
content/assets/theory-images/fowler-harness-templates.png
content/assets/theory-images/fowler-harness-change-lifecycle.png
...
```text
Дополнительный внешний поиск разрешён для primary sources, свежести и real image candidates; новые темы статей не создавать.
```
...
Создай `work/atlas/articles/bmad_method.md` как большую самостоятельную concept-first статью атласа: **BMAD Method**. Не вводи внутренний лимит объёма. Статья должна отвечать на вопрос читателя: как BMAD распределяет агентную разработку между ролями, документами, gates, workflows and story context так, чтобы работа переходила через читаемые, проверяемые и исправляемые артефакты?
...
Reader question: как BMAD распределяет агентную разработку между ролями, документами, gates, workflows and story context так, чтобы работа переходила через читаемые, проверяемые и исправляемые артефакты?

Граница статьи: BMAD ≠ GSD: artifact-first SDLC profile vs runtime/process supervision. BMAD ≠ PWG: documents/roles/workflows vs durable graph state. BMAD ≠ Gas Town: project methodology vs operating environment. BMAD ≠ Spec Kit/Kiro/SPDD/TDAD: full SDLC profile vs specification/test methods. ADR stores decisions, not the whole execution method.

Отрицательная граница: Статья не должна стать каталогом personas/commands, ecosystem overview or generic SDLC guide. Каждая деталь BMAD должна показывать функцию в lifecycle: creates next input, narrows implementation context, records state, gives human decision point, produces evidence or repair path.
...
Сделай inventory досье: что уже перенесено, что осталось неперенесённым, что требует открытия первоисточников, где у будущей статьи могут не хватать технические опоры. Обнови основной текст только при явном пропуске article-critical материала. Запиши результат в `bmad_method_source_transfer_ledger.md` and `bmad_method_source_usage.md`.

Не строй тотальную coverage matrix по всему досье. Ledger должен помогать статье, а не заменять её.
```
### P04 — source-depth 1: BMAD as documentary handoff
```text
Прочитай сначала:
- work/atlas/articles/bmad_method.md
- work/dossiers/BMAD_METHOD_DOSSIER.md
...
Раскрой core chain of artifacts and roles as context handoff, not persona catalog.


Синхронизация companion-файлов обязательна: если проход добавляет, переносит, отклоняет или переставляет фактуру, источник, границу, изображение, схему, термин или тезис, обнови `source_usage`, `source_transfer_ledger`, `image_plan`, `external_image_queue`, `open_questions`, `theory_links` и audit/readiness-файлы.
```
### P05 — source-depth 2: file mechanics and recovery surface
```text
Прочитай сначала:
- work/atlas/articles/bmad_method.md
- work/dossiers/BMAD_METHOD_DOSSIER.md
...
Раскрой `_bmad/`, `_bmad-output/`, manifests, story files, sprint state and project-context as surfaces for recovery and next action.


Синхронизация companion-файлов обязательна: если проход добавляет, переносит, отклоняет или переставляет фактуру, источник, границу, изображение, схему, термин или тезис, обнови `source_usage`, `source_transfer_ledger`, `image_plan`, `external_image_queue`, `open_questions`, `theory_links` и audit/readiness-файлы.
```
### P06 — source-depth 3: brownfield/document-project
```text
Прочитай сначала:
- work/atlas/articles/bmad_method.md
- work/dossiers/BMAD_METHOD_DOSSIER.md
...
Покажи scan levels, documentation requirements, project-scan report, generated docs/index and stale-doc risk as knowledge creation for future agents.


Синхронизация companion-файлов обязательна: если проход добавляет, переносит, отклоняет или переставляет фактуру, источник, границу, изображение, схему, термин или тезис, обнови `source_usage`, `source_transfer_ledger`, `image_plan`, `external_image_queue`, `open_questions`, `theory_links` и audit/readiness-файлы.
```
### P07 — source-depth 4: review, correct-course, human gates
```text
Прочитай сначала:
- work/atlas/articles/bmad_method.md
- work/dossiers/BMAD_METHOD_DOSSIER.md
...
Раскрой PRD validation, readiness, checkpoint preview, investigation, correct-course, retrospective and where humans decide.


Синхронизация companion-файлов обязательна: если проход добавляет, переносит, отклоняет или переставляет фактуру, источник, границу, изображение, схему, термин или тезис, обнови `source_usage`, `source_transfer_ledger`, `image_plan`, `external_image_queue`, `open_questions`, `theory_links` и audit/readiness-файлы.
```
### P08 — source-depth 5: evidence/release extensions and failure modes
```text
Прочитай сначала:
- work/atlas/articles/bmad_method.md
- work/dossiers/BMAD_METHOD_DOSSIER.md
...
Раскрой TEA/Enterprise/release gate only as extension when supported by dossier; add limits: token cost, overplanning, stale artifacts, excessive ceremony, module confusion.
...
Проведи local asset pass. Правило: relevant local assets работают по insert-or-explain. Если локальный asset релевантен статье, вставь его по месту применения как `<figure><img src="...">...</figure>` с публичной подписью; если не вставляешь — запиши причину в `work/atlas/articles/bmad_method_image_plan.md`.

Visual priorities для этой статьи: Приоритетны real workflow maps, role/artifact diagrams, file tree `_bmad/` + `_bmad-output/`, sprint-status state machine, brownfield scan/document-project flow, checkpoint preview/correct-course surfaces, TEA/release-gate visuals. Synthetic diagrams only for artifact handoff or BMAD/GSD/PWG/Gas Town boundaries.
...
Усиль статью как standalone concept-first article. Читатель должен понять **BMAD Method** без предварительной сборки материала из всех глав теории. Контролируемые повторы с теорией допустимы, если нужны для понимания конкретной концепции; механический copy-paste общей теории запрещён.
...
Особенно проверь: personas become theater; documents do not become next-step inputs; stale artifacts; Quick Flow/Full Method/Enterprise confused; overplanning; evidence layer treated as mandatory core; BMAD duplicates PWG/GSD/Gas Town.
