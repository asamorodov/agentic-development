# Target-group plan: статья атласа `constitutional_sdd` — Constitutional SDD

Статус: `ready_for_package_manufacture_after_manual_review`.  
Article id: `constitutional_sdd`  
Readiness status: `ready_for_package_manufacture_after_manual_review`.  
Рабочее название: `Constitutional SDD`  
...
Вопрос читателя: как constitution/security-policy слой поднимает ограничения над локальной feature-specification и что реально делают compliance matrix, context files and traceability?

Центральная ось статьи: Constitutional SDD важно не лозунгом “security matters”, а рабочей попыткой поднять требования безопасности и политики выше локальной фичи: constitution, spec, plan, tasks, compliance matrix, context files and traceability должны влиять на движение работы, иначе они остаются риторикой.

Соседние границы: CSDD ≠ Spec Kit with security words: Spec Kit даёт repo workflow; CSDD добавляет policy/constitution layer. CSDD ≠ ADR: ADR хранит принятое решение; constitution задаёт ограничения выше feature. CSDD ≠ TDAD/A7: compliance matrix не равна evidence. CSDD ≠ обычная security documentation, если слой не влияет на action.

Визуальный слой: Приоритетны реальные paper/repo figures/tables: constitution snippets, compliance matrix, traceability table, context-file examples, architecture/workflow diagrams. Локальный `fowler-sdd-overview.png` — только для общего SDD context, если уместен.
...
Статья не должна стать security manifesto, пересказом paper/repo или generic governance article. Она должна отделить рабочий constitutional layer от декларации и от обычной документации безопасности.

## 2.1. Фактура без coverage-бюрократии

Constitutional SDD-статья опирается на досье и первоисточники, но package не должен снова превращать перенос фактуры в coverage/disposition-бюрократию. Жёсткий режим `dossier-backed completeness / relevant but untransferred` считается откатанным: не нужно строить тотальную matrix по всему досье и доказывать статус каждого релевантного фрагмента.

Рабочий ориентир проще: ключевые тезисы статьи должны иметь достаточные технические опоры. Для этой статьи особенно важны: policy/constitution files, checks, CI/security constraints, exception handling, and where human authority remains required. Если без такой детали раздел остаётся общей прозой, материал лучше встроить в основной текст. Если деталь не помогает reader question, относится к соседней статье или ломает последовательность, её не нужно вставлять ради отчётности.
...
1. **Локальная feature vs ограничения выше feature.**
2. **Что в paper/repo является workflow, а что academic framing.**
3. **Constitution/spec/plan/tasks/compliance matrix/context files.**
4. **Traceability and compliance as useful constraint and formal-risk.**
5. **Связь с Spec Kit and specification methods.**
6. **Где human judgment and security responsibility remain.**
7. **Failure modes: declaration, formal matrix, false compliance, no action change.**
...
```yaml
- path: work/atlas/articles/constitutional_sdd.md
  status: future
  role: primary
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Constitutional SDD atlas article."
- path: work/atlas/articles/constitutional_sdd_source_usage.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Constitutional SDD atlas article."
- path: work/atlas/articles/constitutional_sdd_source_transfer_ledger.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Constitutional SDD atlas article."
- path: work/atlas/articles/constitutional_sdd_image_plan.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Constitutional SDD atlas article."
- path: work/atlas/articles/constitutional_sdd_external_image_queue.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Constitutional SDD atlas article."
- path: work/atlas/articles/constitutional_sdd_open_questions.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Constitutional SDD atlas article."
- path: work/atlas/articles/constitutional_sdd_theory_links.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Constitutional SDD atlas article."
- path: work/atlas/articles/constitutional_sdd_degradation_and_duplication_audit.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Constitutional SDD atlas article."
- path: work/atlas/articles/constitutional_sdd_readiness_report.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
...
https://ar5iv.labs.arxiv.org/html/2602.02584v1
https://arxiv.org/pdf/2602.02584
https://raw.githubusercontent.com/srinivasraom/banking-ms-by-constitution/001-banking-crud/PAPER.md
https://maddevs.io/writeups/practical-security-layer-for-spec-driven-development/
https://github.com/github/spec-kit/blob/main/spec-driven.md
https://github.com/github/spec-kit/issues/697
https://github.com/github/spec-kit/issues/40
https://github.com/raccioly/docguard
https://pypi.org/project/docguard-cli/0.24.0/
...
https://raw.githubusercontent.com/srinivasraom/banking-ms-by-constitution/001-banking-crud/specs/001-banking-crud/spec.md
https://raw.githubusercontent.com/srinivasraom/banking-ms-by-constitution/001-banking-crud/specs/001-banking-crud/plan.md
https://raw.githubusercontent.com/srinivasraom/banking-ms-by-constitution/001-banking-crud/specs/001-banking-crud/tasks.md
...
https://raw.githubusercontent.com/srinivasraom/banking-ms-by-constitution/001-banking-crud/CLAUDE.md
...
Создай `work/atlas/articles/constitutional_sdd.md` как большую самостоятельную concept-first статью атласа: **Constitutional SDD**. Не вводи внутренний лимит объёма. Статья должна отвечать на вопрос читателя: как constitution/security-policy слой поднимает ограничения над локальной feature-specification и что реально делают compliance matrix, context files and traceability?
...
Reader question: как constitution/security-policy слой поднимает ограничения над локальной feature-specification и что реально делают compliance matrix, context files and traceability?

Граница статьи: CSDD ≠ Spec Kit with security words: Spec Kit даёт repo workflow; CSDD добавляет policy/constitution layer. CSDD ≠ ADR: ADR хранит принятое решение; constitution задаёт ограничения выше feature. CSDD ≠ TDAD/A7: compliance matrix не равна evidence. CSDD ≠ обычная security documentation, если слой не влияет на action.

Отрицательная граница: Статья не должна стать security manifesto, пересказом paper/repo или generic governance article. Она должна отделить рабочий constitutional layer от декларации и от обычной документации безопасности.
...
- work/atlas/articles/constitutional_sdd.md
- work/dossiers/CONSTITUTIONAL_SDD_DOSSIER.md
- work/atlas/articles/constitutional_sdd_source_transfer_ledger.md
- work/atlas/articles/constitutional_sdd_image_plan.md
- protocols/rules/source-and-provenance.md
- protocols/rules/content-preservation.md

Сделай inventory досье: что уже перенесено, что осталось неперенесённым, что требует открытия первоисточников, где у будущей статьи могут не хватать технические опоры. Обнови основной текст только при явном пропуске article-critical материала. Запиши результат в `constitutional_sdd_source_transfer_ledger.md` and `constitutional_sdd_source_usage.md`.

Не строй тотальную coverage matrix по всему досье. Ledger должен помогать статье, а не заменять её.
```
### P04 — source-depth 1: workflow vs academic framing
```text
Прочитай сначала:
- work/atlas/articles/constitutional_sdd.md
- work/dossiers/CONSTITUTIONAL_SDD_DOSSIER.md
...
Раздели paper framing and repository workflow: что реально может выполнить agent/package, а что только объяснительная рамка.
...
Раскрой concrete artifacts: constitution, spec, plan, tasks, context files, generated/expected outputs and state.


Синхронизация companion-файлов обязательна: если проход добавляет, переносит, отклоняет или переставляет фактуру, источник, границу, изображение, схему, термин или тезис, обнови `source_usage`, `source_transfer_ledger`, `image_plan`, `external_image_queue`, `open_questions`, `theory_links` и audit/readiness-файлы.
```
### P06 — source-depth 3: compliance matrix and traceability
```text
Прочитай сначала:
- work/atlas/articles/constitutional_sdd.md
- work/dossiers/CONSTITUTIONAL_SDD_DOSSIER.md
...
Покажи compliance matrix and traceability как working constraint: когда они влияют на plan/tasks/review, а когда остаются формальной таблицей.


Синхронизация companion-файлов обязательна: если проход добавляет, переносит, отклоняет или переставляет фактуру, источник, границу, изображение, схему, термин или тезис, обнови `source_usage`, `source_transfer_ledger`, `image_plan`, `external_image_queue`, `open_questions`, `theory_links` и audit/readiness-файлы.
```
### P07 — source-depth 4: связь и границы со Spec Kit and security documentation
```text
Прочитай сначала:
- work/atlas/articles/constitutional_sdd.md
- work/dossiers/CONSTITUTIONAL_SDD_DOSSIER.md
...
Покажи CSDD as constitutional layer over/near specification workflow; не смешивай с Spec Kit, ADR or generic security docs.
...
Добавь limits: constitution as declaration, compliance matrix as overconfidence, policy not connected to action, missing evidence, security responsibility still human.
...
Проведи local asset pass. Правило: relevant local assets работают по insert-or-explain. Если локальный asset релевантен статье, вставь его по месту применения как `<figure><img src="...">...</figure>` с публичной подписью; если не вставляешь — запиши причину в `work/atlas/articles/constitutional_sdd_image_plan.md`.

Visual priorities для этой статьи: Приоритетны реальные paper/repo figures/tables: constitution snippets, compliance matrix, traceability table, context-file examples, architecture/workflow diagrams. Локальный `fowler-sdd-overview.png` — только для общего SDD context, если уместен.
...
Усиль статью как standalone concept-first article. Читатель должен понять **Constitutional SDD** без предварительной сборки материала из всех глав теории. Контролируемые повторы с теорией допустимы, если нужны для понимания конкретной концепции; механический copy-paste общей теории запрещён.
...
- work/atlas/articles/constitutional_sdd.md
- work/dossiers/CONSTITUTIONAL_SDD_DOSSIER.md
- work/atlas/articles/constitutional_sdd_degradation_and_duplication_audit.md

Усиль механизм, границы и failure modes. Не делай отдельный каталог ошибок: встрой ограничения в объяснение концепции.

Особенно проверь: constitution becomes declaration; compliance matrix looks stronger than it is; security policy not connected to next action; traceability without evidence; model optimizes checklist wording; article becomes governance rhetoric.
...
- work/atlas/articles/constitutional_sdd_image_plan.md
- work/atlas/articles/constitutional_sdd_degradation_and_duplication_audit.md
- work/atlas/articles/constitutional_sdd_readiness_report.md
