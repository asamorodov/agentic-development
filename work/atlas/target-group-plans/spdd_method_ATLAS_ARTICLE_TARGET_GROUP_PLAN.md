...
SPDD-статья опирается на досье и первоисточники, но package не должен снова превращать перенос фактуры в coverage/disposition-бюрократию. Для этого плана жёсткий `dossier-backed completeness / relevant but untransferred` gate считается откатанным: не нужно строить тотальную matrix по всему досье и доказывать статус каждого релевантного фрагмента.

Мягкий ориентир остаётся: ключевые тезисы статьи должны иметь технические опоры. Если раздел объясняет REASONS Canvas, OpenSPDD workflow, evidence/review, `prompt-update`, `sync`, failure mode или границу SPDD, читателю обычно нужны конкретные детали: команда, шаблон, пример, число, визуальный кандидат, ограничение или source-specific наблюдение. Если без такой опоры раздел звучит общей прозой, деталь лучше встроить в основной текст. Если деталь не помогает reader question, относится к соседней статье или ломает ход объяснения, её не нужно вставлять ради отчётности.
...
Сделай inventory досье: что уже перенесено, что осталось неперенесённым, что требует открытия первоисточников, где у будущей статьи могут не хватать технические опоры. Обнови основной текст только при явном пропуске article-critical материала. Запиши результат в `spdd_method_source_transfer_ledger.md` and `spdd_method_source_usage.md`.
...
Проверь, не осталась ли статья конспектом. Особенно ищи места, где текст говорит «SPDD синхронизирует намерение и код», «Canvas обновляется», «ревью обнаруживает проблему» или «OpenSPDD поддерживает workflow», но не показывает, как это происходит в источнике. Если ключевому тезису не хватает технической опоры, добавь конкретную деталь из источника: команду, шаблон, пример, число, визуальный кандидат, ограничение или наблюдение. Сохрани фактуру, команды, версии, ограничения и источниковую конкретику.
...
Сам определи главный недобор фактуры или технических опор в статье, добавь материал в текст и companion-файлы. В `spdd_method_source_transfer_ledger.md` запиши: что счёл главным недобором, что добавил, откуда взял материал, что осталось долгом.

Не превращай проход в преждевременную стилистическую шлифовку краткого обзора.
...
Проверь, не стала ли статья второй теорией, справочником команд, пересказом досье, набором картинок или кратким конспектом. Дополнительно проверь, хватает ли ключевым тезисам технических опор в основном тексте. Исправь только реальные дефекты и зафиксируй readiness.
```
...
Проверь структуру публичной статьи: H1 должен называть объект статьи, первые абзацы должны сначала объяснять проблему и рабочий механизм SPDD, а taxonomy/atlas-boundary не должны перегружать вход раньше, чем читатель получил опору. Проверь подзаголовки, путь читателя, placement фигур, подписи и отсутствие executor meta-text. Добавляй или убирай подзаголовки только если это помогает видеть смысловые границы. Не превращай текст в список.
```
...
- source-depth and free-expansion проходы действительно перенесли фактуру, которая помогает понять механизм, границы, техническую конкретику и ограничения SPDD.
...
- P23 синхронизирует все companion outputs;
...
Этот manifest разворачивает очередь будущего article-writing package в отдельные prompt records. При сборке executor-пакета нельзя оставлять сгруппированные диапазоны одним record: каждый пункт ниже должен стать отдельным gated prompt.
...
The article-specific queue above this manifest may describe passes in article-specific language, but package manufacturing must expand them according to this manifest and preserve the SPDD-specific emphasis written in the queue and Article contract.
