# Target-group plan: статья атласа `spec_kit_method` — Spec Kit

Статус: `ready_for_package_manufacture_after_manual_review`.  
Article id: `spec_kit_method`  
Readiness status: `ready_for_package_manufacture_after_manual_review`.  
Рабочее название: `Spec Kit`  
Уровень статьи: `методологическая статья о форме инструмента`  
...
Вопрос читателя: как Spec Kit превращает намерение о feature в репозиторный переход между артефактами — specification, plan, tasks, implementation, constitution/checklists — и где этот переход действительно останавливает агента для человеческого решения?

Центральная ось статьи: Spec Kit важен не как набор slash-команд, а как способ сделать specification-driven work исполнимой в репозитории: `/specify`, `/plan`, `/tasks`, `/implement`, constitution, checklist, scripts и agent integrations должны показывать, что переносится из разговора в файлы, а что остаётся инженерным суждением.

Соседние границы: Spec Kit ≠ SPDD/OpenSPDD: SPDD делает prompt/Canvas сопровождаемым артефактом намерения; Spec Kit строит repo workflow вокруг feature. Spec Kit ≠ Kiro: Kiro — productized IDE/Web/CLI surface; Spec Kit — переносимая repo/CLI discipline. Spec Kit ≠ Constitutional SDD: constitution здесь часть workflow, а не вся security-policy рамка. Spec Kit ≠ PWG: он создаёт артефакты фичи, но не обязан хранить весь граф состояния работы.

Визуальный слой: Приоритетны реальные Spec Kit diagrams/screenshots из документации и repo examples: feature workflow, command templates, constitution/checklist, generated plan/tasks, agent integration. `fowler-sdd-overview.png` можно использовать только как SDD-context, а не замену Spec Kit-specific изображениям.
...
Статья не должна стать CLI reference, changelog, installation guide, общим обзором SDD или повтором SPDD/Kiro. Она должна объяснять repo-workflow: когда одного spec-файла недостаточно и как переход между артефактами меняет допустимое следующее действие агента.

## 2.1. Фактура без coverage-бюрократии

Spec Kit-статья опирается на досье и первоисточники, но package не должен снова превращать перенос фактуры в coverage/disposition-бюрократию. Жёсткий режим `dossier-backed completeness / relevant but untransferred` считается откатанным: не нужно строить тотальную matrix по всему досье и доказывать статус каждого релевантного фрагмента.

Рабочий ориентир проще: ключевые тезисы статьи должны иметь достаточные технические опоры. Для этой статьи особенно важны: Spec Kit commands, generated spec/plan/tasks files, constitution/checklist constraints, repo integration details and human decision stops. Если без такой детали раздел остаётся общей прозой, материал лучше встроить в основной текст. Если деталь не помогает reader question, относится к соседней статье или ломает последовательность, её не нужно вставлять ради отчётности.
...
1. **Зачем одного spec-файла недостаточно.**
2. **Переход `/specify → /plan → /tasks → /implement` как последовательность артефактов.**
3. **Constitution Check, clarify/analyze/checklist как условия движения дальше.**
4. **Файлы, scripts, integrations, `AGENTS.md` и repo state.**
5. **Human confirmations: где человек утверждает смысл, план, компромисс и достаточность проверки.**
6. **Границы с SPDD, Kiro, CSDD, TDAD, ADR и PWG.**
7. **Failures: красивый spec без исполнения, декоративный plan, tasks без ответственности, checklist без влияния на решение.**
...
```yaml
- path: work/atlas/articles/spec_kit_method.md
  status: future
  role: primary
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Spec Kit atlas article."
- path: work/atlas/articles/spec_kit_method_source_usage.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Spec Kit atlas article."
- path: work/atlas/articles/spec_kit_method_source_transfer_ledger.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Spec Kit atlas article."
- path: work/atlas/articles/spec_kit_method_image_plan.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Spec Kit atlas article."
- path: work/atlas/articles/spec_kit_method_external_image_queue.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Spec Kit atlas article."
- path: work/atlas/articles/spec_kit_method_open_questions.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Spec Kit atlas article."
- path: work/atlas/articles/spec_kit_method_theory_links.md
  status: future
  role: supporting-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Spec Kit atlas article."
- path: work/atlas/articles/spec_kit_method_degradation_and_duplication_audit.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
  notes: "Future output for the Spec Kit atlas article."
- path: work/atlas/articles/spec_kit_method_readiness_report.md
  status: future
  role: diagnostic-output
  write_policy: replace-full-file
  result_policy: overlay-path
...
https://raw.githubusercontent.com/github/spec-kit/main/scripts/bash/create-new-feature.sh
https://raw.githubusercontent.com/github/spec-kit/main/scripts/bash/setup-plan.sh
https://raw.githubusercontent.com/github/spec-kit/main/scripts/bash/setup-tasks.sh
https://raw.githubusercontent.com/github/spec-kit/main/src/specify_cli/integrations/catalog.py
https://raw.githubusercontent.com/github/spec-kit/main/src/specify_cli/integrations/codex/__init__.py
...
Создай `work/atlas/articles/spec_kit_method.md` как большую самостоятельную concept-first статью атласа: **Spec Kit**. Не вводи внутренний лимит объёма. Статья должна отвечать на вопрос читателя: как Spec Kit превращает намерение о feature в репозиторный переход между артефактами — specification, plan, tasks, implementation, constitution/checklists — и где этот переход действительно останавливает агента для человеческого решения?
...
Reader question: как Spec Kit превращает намерение о feature в репозиторный переход между артефактами — specification, plan, tasks, implementation, constitution/checklists — и где этот переход действительно останавливает агента для человеческого решения?

Граница статьи: Spec Kit ≠ SPDD/OpenSPDD: SPDD делает prompt/Canvas сопровождаемым артефактом намерения; Spec Kit строит repo workflow вокруг feature. Spec Kit ≠ Kiro: Kiro — productized IDE/Web/CLI surface; Spec Kit — переносимая repo/CLI discipline. Spec Kit ≠ Constitutional SDD: constitution здесь часть workflow, а не вся security-policy рамка. Spec Kit ≠ PWG: он создаёт артефакты фичи, но не обязан хранить весь граф состояния работы.

Отрицательная граница: Статья не должна стать CLI reference, changelog, installation guide, общим обзором SDD или повтором SPDD/Kiro. Она должна объяснять repo-workflow: когда одного spec-файла недостаточно и как переход между артефактами меняет допустимое следующее действие агента.
...
Сделай inventory досье: что уже перенесено, что осталось неперенесённым, что требует открытия первоисточников, где у будущей статьи могут не хватать технические опоры. Обнови основной текст только при явном пропуске article-critical материала. Запиши результат в `spec_kit_method_source_transfer_ledger.md` and `spec_kit_method_source_usage.md`.

Не строй тотальную coverage matrix по всему досье. Ledger должен помогать статье, а не заменять её.
```
### P04 — source-depth 1: почему одного spec-файла недостаточно
```text
Прочитай сначала:
- work/atlas/articles/spec_kit_method.md
- work/dossiers/SPEC_KIT_METHOD_DOSSIER.md
...
Покажи, почему Spec Kit строит не один документ, а переходы между specification, plan, tasks и implementation. Раскрой места, где незавершённый вопрос, constraint или missing decision должны остановить движение дальше.


Синхронизация companion-файлов обязательна: если проход добавляет, переносит, отклоняет или переставляет фактуру, источник, границу, изображение, схему, термин или тезис, обнови `source_usage`, `source_transfer_ledger`, `image_plan`, `external_image_queue`, `open_questions`, `theory_links` и audit/readiness-файлы.
```
### P05 — source-depth 2: команды как переходы между артефактами
```text
Прочитай сначала:
- work/atlas/articles/spec_kit_method.md
- work/dossiers/SPEC_KIT_METHOD_DOSSIER.md
...
Раскрой `/specify`, `/plan`, `/tasks`, `/implement`, `/clarify`, `/analyze`, `/checklist`, `/constitution` не как список команд, а как state transitions в repo workflow. Перенеси детали templates, scripts, created files and expected handoffs.


Синхронизация companion-файлов обязательна: если проход добавляет, переносит, отклоняет или переставляет фактуру, источник, границу, изображение, схему, термин или тезис, обнови `source_usage`, `source_transfer_ledger`, `image_plan`, `external_image_queue`, `open_questions`, `theory_links` и audit/readiness-файлы.
```
### P06 — source-depth 3: Constitution Check и checklist как рабочие ограничения
```text
Прочитай сначала:
- work/atlas/articles/spec_kit_method.md
- work/dossiers/SPEC_KIT_METHOD_DOSSIER.md
...
Покажи, что constitution/checklist полезны только если меняют план, tasks, implementation или human review. Укажи, где они помогают, а где становятся формальностью.


Синхронизация companion-файлов обязательна: если проход добавляет, переносит, отклоняет или переставляет фактуру, источник, границу, изображение, схему, термин или тезис, обнови `source_usage`, `source_transfer_ledger`, `image_plan`, `external_image_queue`, `open_questions`, `theory_links` и audit/readiness-файлы.
```
### P07 — source-depth 4: integrations, presets, AGENTS.md и agent-readable repo surface
```text
Прочитай сначала:
- work/atlas/articles/spec_kit_method.md
- work/dossiers/SPEC_KIT_METHOD_DOSSIER.md
...
Раскрой agent integrations, presets/extensions, `AGENTS.md`, scripts and repository conventions как поверхность, которую может прочитать следующий агент.
...
Найди места, где статья ещё звучит как “Spec Kit ведёт от intent к implementation”, но не показывает, как именно это происходит в файлах и командах. Добавь limits, caveats, issues/PR details and evidence boundaries.
...
Проведи local asset pass. Правило: relevant local assets работают по insert-or-explain. Если локальный asset релевантен статье, вставь его по месту применения как `<figure><img src="...">...</figure>` с публичной подписью; если не вставляешь — запиши причину в `work/atlas/articles/spec_kit_method_image_plan.md`.

Visual priorities для этой статьи: Приоритетны реальные Spec Kit diagrams/screenshots из документации и repo examples: feature workflow, command templates, constitution/checklist, generated plan/tasks, agent integration. `fowler-sdd-overview.png` можно использовать только как SDD-context, а не замену Spec Kit-specific изображениям.
...
Усиль статью как standalone concept-first article. Читатель должен понять **Spec Kit** без предварительной сборки материала из всех глав теории. Контролируемые повторы с теорией допустимы, если нужны для понимания конкретной концепции; механический copy-paste общей теории запрещён.
...
Особенно проверь: красивый spec без executable path; план как декоративный мост; tasks как механическая нарезка без ответственности; constitution/checklist как галочка; checklist/evidence выглядит убедительнее, чем фактически проверяет; Spec Kit применён там, где достаточно лёгкого плана.
